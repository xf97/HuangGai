...
// File: contracts\interfaces\defi\ICurveFiSwap_SBTC.sol
...
// File: contracts\modules\defi\CurveFiProtocol_SBTC.sol
...
/**
 * @dev CurveFi sBTC Pool does not use Deposit contract, co we have to use Swap directly.
...
contract CurveFiProtocol_SBTC is CurveFiProtocol {
    ...
    function setCurveFi(address swap, address liquidityGauge) public onlyDefiOperator {
        ...
        if (address(swap) != address(0)) {
            ...
            for (uint256 i=0; i < _registeredTokens.length; i++){
                if (_registeredTokens[i] != address(0)) {
                    _unregisterToken_SBTC(_registeredTokens[i]);
                    _registeredTokens[i] = address(0);
                }
            }
        }
    ...
    function deposit_remove_liquidity_one_coin(uint256 _token_amount, uint256 i, uint256 min_uamount) internal {
        ...
        amnts[i] = min_uamount;
        ICurveFiSwap_SBTC(address(curveFiSwap)).remove_liquidity_imbalance(amnts, _token_amount);
    }

    function _registerToken_SBTC(address token, uint256 idx) private {
        _registeredTokens[idx] = token;
        IERC20 ltoken = IERC20(token);
    ...
    function _unregisterToken_SBTC(address token) private {
        uint256 balance = IERC20(token).balanceOf(address(this));
    ...
    function deposit_add_liquidity(uint256[] memory amounts, uint256 min_mint_amount) internal {
        ICurveFiSwap_SBTC(address(curveFiSwap)).add_liquidity(convertArray(amounts), min_mint_amount);
    }

    function deposit_remove_liquidity_imbalance(uint256[] memory amounts, uint256 max_burn_amount) internal {
        ICurveFiSwap_SBTC(address(curveFiSwap)).remove_liquidity_imbalance(convertArray(amounts), max_burn_amount);
    }
