...
contract FITHTokenSaleRefAndPromo is FITHTokenSaleReferrals
{
	...
	function getEthUsdPrice() public returns(uint) {
		return OrFeedInterface(orFeedContractAddress).getExchangeRate("ETH", "USDT", "DEFAULT", oneEthAsWei);
	}
	
	function getEthWeiAmountPrice(uint ethWeiAmount) public returns(uint) {
	...
	//---
	function checkEthWeiPromoCost(uint ethWei) public returns(uint) {
	...
	function calculateEthWeiCost(uint ethWei) public returns(uint256) {
