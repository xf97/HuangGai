...
// File: src/UniPriceRebaseInvoker.sol
...
contract UniPriceRebaseInvoker is Ownable {
    ...
    IRebaseToken private _rebaseToken;
    ...
    function _rebase(uint256 epoch) private {
        uint256 usdtAmount = _usdt.balanceOf(address(_pair));
        uint256 rebaseTokenAmount = _rebaseToken.balanceOf(address(_pair));
        uint256 price = rebaseTokenAmount.div(usdtAmount);
        uint256 targerPrice = 10**12;
        uint256 denumerator;
        if (targerPrice < price) {
            denumerator = price.sub(price.sub(targerPrice).mul(90).div(100));
        } else {
            denumerator = price.add(targerPrice.sub(price).mul(90).div(100));
        }
        _rebaseToken.rebase(epoch, targerPrice, denumerator);
        _pair.sync();
    }
    ...
    function rebaseToken() public view returns (IRebaseToken) {
        return _rebaseToken;
    }
