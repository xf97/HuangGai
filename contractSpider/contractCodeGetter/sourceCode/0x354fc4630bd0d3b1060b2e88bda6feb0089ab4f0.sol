...
contract BounceLockingToken is ERC20, OpenZeppelinUpgradesOwnable {
	...
	constructor() ERC20("Bounce Locking Token", "BOLT") public {
		_setupDecimals(18);
	}
	...
	function startUnlock(address _token, address recipient, uint _begin, uint _span, uint _times) external onlyOwner {
	    token = _token;
		_setupDecimals(ERC20(token).decimals());
	    _mint(recipient, ERC20(token).balanceOf(address(this)).sub(_totalSupply));
    ...
    function unlockCapacity(address holder) public view returns (uint) {
       if(begin == 0 || now <= begin)
        ...
        for(uint i=1; i<times; i++)
            if(now < span.mul(i).div(times.sub(1)).add(begin))
                return _balances[holder].sub(_beginBalances[holder].mul(times.sub(i)).div(times));
