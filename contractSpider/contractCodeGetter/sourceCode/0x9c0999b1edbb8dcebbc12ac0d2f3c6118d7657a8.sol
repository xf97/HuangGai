 ...
 */
contract Cradle is IStaking, Ownable {
    ...
    constructor(IERC20 stakingToken, IERC20 distributionToken, uint256 maxUnlockSchedules,
        ...
        require(bonusPeriodSec_ != 0, 'Cradle: bonus period is zero');
        require(initialSharesPerToken > 0, 'Cradle: initialSharesPerToken is zero');
    ...
    function _stakeFor(address staker, address beneficiary, uint256 amount) private {
        require(amount > 0, 'Cradle: stake amount is zero');
        ...
        require(mintedStakingShares > 0, 'Cradle: Stake amount is too small');
    ...
    function _unstake(uint256 amount) private returns (uint256) {
        ...
        // checks
        require(amount > 0, 'Cradle: unstake amount is zero');
        require(totalStakedFor(msg.sender) >= amount,
            'Cradle: unstake amount is greater than total user stakes');
        uint256 stakingSharesToBurn = totalStakingShares.mul(amount).div(totalStaked());
        require(stakingSharesToBurn > 0, 'Cradle: Unable to unstake amount this small');
    ...
    function lockTokens(uint256 amount, uint256 durationSec) external onlyOwner {
        require(unlockSchedules.length < _maxUnlockSchedules,
            'Cradle: reached maximum unlock schedules');
