...
contract LexAIR {
    ...
    event Register(address indexed _account);
    event Deregister(address indexed _account);
    ...
    LISTING FUNCTIONS
    ****************/
    function delist(address[] calldata _account) external {
        ...
        for (uint256 i = 0; i < _account.length; i++) {
            ...
            emit Deregister(_account[i]);
        }
    }
    
    function list(address[] calldata _account) external { 
        require(IERC20(accessToken).balanceOf(msg.sender) >= 1, "!access");
