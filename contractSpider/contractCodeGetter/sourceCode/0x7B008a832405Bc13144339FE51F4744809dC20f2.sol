...
contract SimpleTokenCoin is MintableToken {
    string public constant name = "utrade.cash";

    string public constant symbol = "UTRD";
...
contract ACrowdsale is Ownable{
    ...
    function() external payable {
        ...
        if(token.mintingFinished() || isFinished){revert("ICO is CLOSED");}
        if(now < start  || ICOtimeLeftMinsICO() <= 0){revert("ICO interval error");}
        if(!(influences[msg.sender] > 0)){revert("You not influence");}
        if(maxSellingInICO <= token.totalSupply()){revert("Ico tokens run out");}
        if(influences[msg.sender] < (msg.value + token.balanceOf(msg.sender))){revert("Your limit has been exceeded");}
        if(amount != 1 * sat && amount != 2 * sat && amount != 3 * sat){revert("Ceil token only");}

        token.mint(msg.sender, amount);

        if(token.totalSupply() >= maxSellingInICO){
            closeICO();
        }
    }
   ...
   function createInfluencesSecond() public onlyOwner {
    ...
    function closeICO() private {
       isFinished = true;
       token.mint(owner, coinsAfterIcoAmountTeam);
       leftTokensAmountToTeamBlocked = total - token.totalSupply();
