...
interface IMigratorChef {
    ...
    // ShumaiSwap must mint EXACTLY the same amount of ShumaiSwap LP tokens or
    // else something bad will happen. Traditional UniswapV2 does not
    // do that so be careful!
    function migrate(IERC20 token) external returns (IERC20);
}

// ChefTang is the master of Shumai. He can make Shumai and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once Shumai is sufficiently
...
contract  ChefTang is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of Shumais
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. Shumais to distribute per block.
    ...
    // Block number when bonus SFI period ends.
    uint256 public bonusEndBlock;
    // SFI tokens created per block.
    uint256 public sfiPerBlock;
    // Bonus muliplier for early SFI makers.
    ...
    // The block number when SFI mining starts.
    ...
    // Update the given pool's SFI allocation point. Can only be called by the owner.
    ...
    // View function to see pending SFIs on frontend.
    ...
    // Deposit LP tokens to ChefTang for SFI allocation.
    ...
    // Withdraw LP tokens from ChefTang.
