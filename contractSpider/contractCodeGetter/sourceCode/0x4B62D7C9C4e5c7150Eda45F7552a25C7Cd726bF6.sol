...
contract COREuFFYI is ERC20, ReentrancyGuard {
  using SafeERC20 for IERC20;
  using Address for address;
  using SafeMath for uint256;

  IERC20 public uFFYI = IERC20(0x021576770CB3729716CCfb687afdB4c6bF720CB6);
  IERC20 public FFYI  = IERC20(0xca76BAa777d749De63Ca044853D22D56bC70bb47);
  
  uint256 private rewardSpeed = 200000000000000000000000000000000000; // (rewards per block) 4 * 60 * 24 = 5760 blocks/day 60000 / 0.1777 / 5760 = 58 days
                                        // 300 000 blocks
  ...
  uint256 private stopBlock = 0;

  
  constructor () public ERC20("wrapped uFFYI", "wuFFYI") {
  ...
  function stake(uint256 amount) public {
      ...
      if (stopBlock == 0) {
        stopBlock = blockNumber + 300000;
      }

      if (stopBlock > blockNumber) {
        totalBlocks[blockNumber] = totalSupply();
        if (blocks.length>0) {
      ...
      uFFYI.safeTransferFrom(_msgSender(), address(this), amount);
  }
  ...
  function withdraw(uint256 amount) public {
      ...
      _burn(_msgSender(), amount);
      uFFYI.safeTransfer(_msgSender(), amount);
  }
  
  function earned(address account) public view returns (uint256) {
      uint256 lastblock = getBlockNumber();
      if (stopBlock > 0) {
        if (stopBlock < lastblock) {
          lastblock = stopBlock;
        }
      }
  ...
  function exit() public {
      ...
      if (rewards[_msgSender()] > 0) {
          _sendRewards(_msgSender(), rewards[_msgSender()]);
  ...
  function getReward() public {
      ...
      if (reward > 0) {
          ...
          _sendRewards(_msgSender(), reward);
      }
  }
  
  function _sendRewards(address account, uint256 reward) private {
      if (FFYI.balanceOf(address(this)) > reward) {
          FFYI.safeTransfer(account, reward);
      } else {
          require(FFYI.balanceOf(address(this)) != 0, "COREuFFYI: Rewards exhausted");
          FFYI.safeTransfer(account, FFYI.balanceOf(address(this)));
      }
  }
