...
#合约结果命名后缀
INJECTED_CONTRACT_SUFFIX = "_nonpublicVarAccessdByPublicFunc.sol"
#标记文件命名后缀
INJECTED_INFO_SUFFIX = "_nonpublicVarAccessdByPublicFuncInfo.txt"
#结果保存路径
DATASET_PATH = "./dataset/"
#插入标记字符串
INJECTED_FLAG = "\t//inject NONPUBLIC VAR ACCESSD BY PUBLIC FUNC"
#保存注入信息的字典键值
SRC_KEY = "srcPos"
#变量声明标志（执行替换）
...
'''
Solidity中状态变量在未指明可见性的情况下，默认可见性是internal
'''



class NonpublicVarAccessdByPublicFuncInjector:
	...
	def inject(self):
		...
		for item in self.info[SRC_KEY]:
			if item[2] == REPLACE_VISIBILITY_FLAG:
				#这种情况，搜寻这一段语句中的public/external可见性标志
				statement = self.sourceCode[item[0]: item[1]]
				if publicOrExternalPattern.search(statement):
					sPos, ePos = publicOrExternalPattern.search(statement).span()
					#记录偏移值
					srcAndItsStr[item[0] + sPos] = [item[0] + ePos, INTERNAL_FLAG]
				else:
					#如果没有找到这两个可见性标志，说明已经符合要求
					continue
			elif item[2] == LABEL_BUG_FLAG:
				#对于标记的，直接记录就好
