...
#源代码保存路径
CONTRACT_PATH = "../../contractExtractor/unlimitedCompilerVersionsExtractor/result"
#注入信息保存路径
INJECT_INFO_PATH = "../../contractExtractor/unlimitedCompilerVersionsExtractor/injectInfo"
#sol文件后缀
SOL_SUFFIX = ".sol"
#json.ast文件后缀
JSON_AST_SUFFIX = "_json.ast"

from unlimitedCompilerVersionsInjector import unlimitedCompilerVersionsInjector	#注入器
import os
import time

class unlimitedCompilerVersions:
	...
	def run(self):
		...
		for contractFile in self.targetContract:
			...
			try:
				...
				#3. 根据目标路径和源代码注入bug
				UI = unlimitedCompilerVersionsInjector(CACHE_CONTRACT_PATH, CACHE_PATHINFO_PATH, astFile, self.getOriginalContractName(contractFile))
				UI.inject()
				UI.output()
				#4. 输出进度
				self.nowNum += 1
...
if __name__ == "__main__":
	ucv = unlimitedCompilerVersions(INJECT_INFO_PATH, CONTRACT_PATH)
	ucv.run()
