...
#源代码保存路径
CONTRACT_PATH = "../../contractExtractor/lockedEtherExtractor/result"
#注入信息保存路径
INJECT_INFO_PATH = "../../contractExtractor/lockedEtherExtractor/injectInfo"
#sol文件后缀
SOL_SUFFIX = ".sol"
#json.ast文件后缀
JSON_AST_SUFFIX = "_json.ast"

from lockedEtherInjector import lockedEtherInjector	#注入器
import os
import time
	...
	def run(self):
		...
		for contractFile in self.targetContract:
			...
			try:
				...
				#contractFile = os.path.join(CONTRACT_PATH, "0x28b6bf339afd2dceeeebd6b218e798b08f38a2e9.sol")
				...
				#3. 根据目标路径和源代码注入bug
				LI = lockedEtherInjector(CACHE_CONTRACT_PATH, CACHE_PATHINFO_PATH, astFile, self.getOriginalContractName(contractFile))
				LI.inject()
				LI.output()
				#4. 输出进度
				self.nowNum += 1
...
if __name__ == "__main__":
	le = lockedEther(INJECT_INFO_PATH, CONTRACT_PATH)
	le.run()
