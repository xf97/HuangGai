...
#源代码保存路径
CONTRACT_PATH = "../../contractExtractor/shortAddressAttackExtractor/result"
#注入信息保存路径
INJECT_INFO_PATH = "../../contractExtractor/shortAddressAttackExtractor/injectInfo"
#sol文件后缀
SOL_SUFFIX = ".sol"
#json.ast文件后缀
JSON_AST_SUFFIX = "_json.ast"

from shortAddressAttackInjector import shortAddressAttackInjector	#注入器
import os
import time
	...
	def run(self):
		...
		for contractFile in self.targetContract:
			...
			try:
				...
				#3. 根据目标路径和源代码注入bug
				SJ = shortAddressAttackInjector(CACHE_CONTRACT_PATH, CACHE_PATHINFO_PATH, astFile, self.getOriginalContractName(contractFile))
				SJ.inject()
				SJ.output()
				#4. 输出进度
				self.nowNum += 1
