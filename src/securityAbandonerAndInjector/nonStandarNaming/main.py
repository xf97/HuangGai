...
#源代码保存路径
CONTRACT_PATH = "../../contractExtractor/nonStandardNamingExtractor/result"
#注入信息保存路径
INJECT_INFO_PATH = "../../contractExtractor/nonStandardNamingExtractor/injectInfo"
#sol文件后缀
SOL_SUFFIX = ".sol"
#json.ast文件后缀
JSON_AST_SUFFIX = "_json.ast"

from nonStandardNamingInjector import nonStandardNamingInjector	#注入器
import os
import time

class nonStandardNaming:
	...
	def run(self):
		...
		for contractFile in self.targetContract:
			...
			try:
				...
				#3. 根据目标路径和源代码注入bug
				NI = nonStandardNamingInjector(CACHE_CONTRACT_PATH, CACHE_PATHINFO_PATH, astFile, self.getOriginalContractName(contractFile))
				NI.inject()
				NI.output()
				#4. 输出进度
				self.nowNum += 1
...
if __name__ == "__main__":
	nsn = nonStandardNaming(INJECT_INFO_PATH, CONTRACT_PATH)
	nsn.run()
