...
class contractAffectdByMinersExtractor:
	...
	def changeSolcVersion(self, _sourceCode):
		...
		try:
			if self.inStandardVersion(self.defaultSolc):
				#在本机支持的solc版本范围内
				#重新导出输出流，保持终端的整洁
				subprocess.run("solc use " + self.defaultSolc, check = True, shell = True, stdout = subprocess.PIPE, stderr = subprocess.PIPE)	#切换版本			
			else:
