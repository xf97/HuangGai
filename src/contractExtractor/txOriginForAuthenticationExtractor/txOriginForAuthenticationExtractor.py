...
'''
两个抽取标准
1. 合约中使用tx.origin作为身份验证
2. 但是并不是tx.origin == msg.sender的验证
思路转变
1. 合约中使用msg.sender做身份验证，直接做模式匹配就好---msg.sender == owner，但是不是每个开发者都把拥有者叫做owner
   去查找
2. 但是并不是tx.origin == msg.sender的验证
'''
...
class txOriginForAuthenticationExtractor:
