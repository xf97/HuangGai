...
'''
此文件用于从数据集中抽取可能包含或者注入后可以包含预发送以太币bug的合约
'''

'''
抽取标准：
１．合约中存在require()和assert语句
２．语句中存在==符号，一侧是uint256常量且这个常量必须是0
３．那么此时就记录另外其中一侧的源代码位置
'''
...
if __name__ == "__main__":
	#从命令行参数处接收需求数量
	userNeed = sys.argv[1]
	psee = preSentEtherExtractor(int(userNeed))
	psee.extractContracts()
