...
class judgeAst:
	...
	def filterConditionPart(self, _astList):
		...
		for item in _astList:
			...
			#ope1是零常量
			if ope1["name"] == LITERAL_FLAG and ope1["attributes"]["type"].split()[0] == PRE_INT_CONST_STR and self.isZero(ope1):
				result.append(ope2["src"])
			#ope2是零常量
			elif ope2["name"] == LITERAL_FLAG and ope2["attributes"]["type"].split()[0] == PRE_INT_CONST_STR and self.isZero(ope2):
	...
	#该函数用于判定整数型常量值是否是0
	def isZero(self, _ope):
		return _ope["attributes"]["value"] == ZERO_FLAG
