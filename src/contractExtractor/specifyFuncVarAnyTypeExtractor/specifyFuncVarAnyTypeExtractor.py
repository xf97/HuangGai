...
#从数据集中抽取可以被注入 任意指定函数变量类型　bug的合约
...
class specifyFuncVarAnyTypeExtractor:
	def __init__(self, _needsNum = 100):
		self.needs = _needsNum
		self.nowNum = 0
		self.defaultSolc = "0.5.1"	#默认使用的solc编译版本
		self.maxSolc = "0.5.17" #最高被支持的solc版本，合约使用的solc版本高于此版本时，引发异常
