...
public函数未被声明为external的bug
的合约
'''

'''
抽取标准：
１．合约中存在external可见性的函数
２．该函数不能是fallback函数和constructor函数
记录external关键字的位置
'''
...
class publicFuncToExternalExtractor:
	...
	def getSourceCode(self):
		...
		try:
			...
			#sourceCode = open(os.path.join(TESTCASE_PATH, "callData.sol"), "r", encoding = "utf-8").read()
