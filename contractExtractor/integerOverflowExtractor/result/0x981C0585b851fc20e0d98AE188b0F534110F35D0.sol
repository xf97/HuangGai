...
// 
// LemonTree can make Lemon and he is a fair guy.
//
...
contract LemonTree is Ownable {
    ...
    // Lemon supply must be less than max supply!
    uint256 public maxLemonSupply = 10*10**26;
    // When the lemon reaches this level, the yield is halved!
    uint256 public halvingLemonSupply = 5*10**26;
    // The LEMON TOKEN!
    LemonToken public lemon;
    // Dev address.
    address public devaddr;
    // Block number when bonus LEMON period ends.
    ...
    // Update the given pool's LEMON allocation point. Can only be called by the owner.
    ...
    function getMultiplier(uint256 _from, uint256 _to) public view returns (uint256) {
        if( lemon.totalSupply() >= halvingLemonSupply){
    ...
    function updatePool(uint256 _pid) public {
        if (lemon.totalSupply() < maxLemonSupply){
            PoolInfo storage pool = poolInfo[_pid];
    ...
    // Deposit LP tokens to LemonTree for LEMON allocation.
    ...
    // Withdraw LP tokens from LemonTree.
    ...
    function setLemonPerBlock(uint256 _lemonPerBlock) public onlyOwner {
        massUpdatePools();
        lemonPerBlock = _lemonPerBlock;
    }

    // Used for community governance.
    function setHalvingLemonSupply(uint256 _halvingLemonSupply) public onlyOwner {
        massUpdatePools();
        require(lemon.totalSupply() < _halvingLemonSupply, "setHalvingLemonSupply: less than current lemon supply");
        require(maxLemonSupply > _halvingLemonSupply, "setHalvingLemonSupply: less than current max lemon supply");
        halvingLemonSupply = _halvingLemonSupply;
    }

    // Used for community governance.
    function setMaxLemonSupply(uint256 _maxLemonSupply) public onlyOwner {
        massUpdatePools();
        require(lemon.totalSupply() < _maxLemonSupply, "setMaxLemonSupply: less than current lemon supply");
        require(halvingLemonSupply < _maxLemonSupply, "setMaxLemonSupply: less than halve point");
        maxLemonSupply = _maxLemonSupply;
    }
