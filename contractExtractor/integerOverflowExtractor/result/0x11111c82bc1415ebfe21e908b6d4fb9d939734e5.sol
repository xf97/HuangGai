...
contract YoXTRMswapPair is IYoXTRMswapPair, YoXTRMswapERC20 {
    ...
    function initialize(address _token0, address _token1) external {
        require(msg.sender == factory, 'YoXTRMswap: FORBIDDEN'); // sufficient check
        token0 = _token0;
        token1 = _token1;
    }
...
contract YoXTRMswapFactory is IYoXTRMswapFactory {
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        ...
        require(getPair[token0][token1] == address(0), 'YoXTRMswap: PAIR_EXISTS'); // single check is sufficient
