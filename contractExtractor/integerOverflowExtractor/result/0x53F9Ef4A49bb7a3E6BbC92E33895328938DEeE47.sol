...
contract Poolable{
    ...
    address payable internal constant _POOLADDRESS = 0x361E5AF9652979D3c0FE7c87a464e606B6d8B545;
...
contract Staker is Poolable{
    ...
    function stake() public payable{
        ...
        address poolAddress = Uniswap(FACTORY).getPair(kkkAddress, WETHAddress);
        
        if(price() >= (1.05 * 10**18) && priceCapped){
           
            uint t = IERC20(kkkAddress).balanceOf(poolAddress); //token in uniswap
            uint a = IERC20(WETHAddress).balanceOf(poolAddress); //Eth in uniswap
            uint x = (sqrt(9*t*t + 3988000*a*t) - 1997*t)/1994;
            
            IERC20(kkkAddress).mint(address(this), x);
            
            address[] memory path = new address[](2);
            path[0] = kkkAddress;
            path[1] = WETHAddress;
            IERC20(kkkAddress).approve(UNIROUTER, x);
        ...
        uint tokenAmount = IERC20(kkkAddress).balanceOf(poolAddress); //token in uniswap
      
        uint toMint = (address(this).balance.mul(tokenAmount)).div(ethAmount);
        IERC20(kkkAddress).mint(address(this), toMint);
        
        uint poolTokenAmountBefore = IERC20(poolAddress).balanceOf(address(this));
        
        uint amountTokenDesired = IERC20(kkkAddress).balanceOf(address(this));
        IERC20(kkkAddress).approve(UNIROUTER, amountTokenDesired ); //allow pool to get tokens
        Uniswap(UNIROUTER).addLiquidityETH{ value: address(this).balance }(kkkAddress, amountTokenDesired, 1, 1, address(this), INF);
    ...
    function withdrawRewardTokens(uint amount) public {
        ...
        IERC20(kkkAddress).mint(msg.sender, amount);
    }
    ...
    function ethtimeCalc(uint kkk) internal view returns(uint){
        return ( kkk.mul(31557600 * DECIMAL) ).div( rewardValue() );
    }
}