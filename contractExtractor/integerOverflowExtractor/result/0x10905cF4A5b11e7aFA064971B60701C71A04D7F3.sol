...
contract Minter is Ownable {
    ...
    function generate(uint256 amount) public onlyOwner returns(bool){
        require(amount > 0, "minter1generate amount error");
        require(amount <= zild.balanceOf(address(this)), "minter1insufficient balance generates more mines");
        minedAmount = minedAmount.add(amount);
        return true;
    } 
    
    function minter(address _to, uint256 amount) public onlyDev returns(bool){
        require(amount > minHandlingFee.mul(10 ** 18).div(100), "minter1withdrawal amount must be greater than the minimum handling fee");
        require(amount <= minedAmount,"minter1Not so many mined token");
    ...
    function collectHandlingFee(uint256 amount) public onlyAdmin returns(bool){
        require(amount > 0, "minter1collect amount error");
        require(amount <= handlingFeeCollect, "minter1withdrawal amount exceeds collector balance");
