...
contract $YUGEFARM is ReentrancyGuard, Pausable, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public lock = 40320; // stake lock in blocks ~ 40,320 7 days for 15s/block
    ...
    function getLockLeft() public view returns (uint256) {
        if(block.number >= stakeLock[_msgSender()]) {
            return 0;
        } else {
            return stakeLock[_msgSender()].sub(block.number);
        }
    }
