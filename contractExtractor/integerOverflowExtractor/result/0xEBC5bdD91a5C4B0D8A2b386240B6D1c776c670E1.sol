...
This project was forked from the KIMCHI.finance project.

 ,ggggggggggg,   ,ggg,         gg  ,ggggggggggg,
dP"""88""""""Y8,dP""Y8a        88 dP"""88""""""Y8,
Yb,  88      `8bYb, `88        88 Yb,  88      `8b
 `"  88      ,8P `"  88        88  `"  88      ,8P
     88aaaad8P"      88        88      88aaaad8P"
     88"""""         88        88      88""""Y8ba
     88              88        88      88      `8b
     88              88        88      88      ,8P
     88              Y8b,____,d88,     88_____,d8'
     88               "Y888888P"Y8    88888888P"
...
// PubToken
contract PubToken is ERC20("PUB.finance","PUB"), Ownable {
...
contract Bartender is Ownable {
    ...
    // The PUB TOKEN!
    PubToken public pub;
    // Block number when bonus PUB period ends.
    uint256 public bonusEndBlock;
    // PUB tokens created per block.
    uint256 public pubPerBlock;
    // Bonus multiplier for early pub makers.
    uint256 public constant BONUS_MULTIPLIER = 1; // no bonus

    // numerator of the owner fee
    uint256 public constant OWNER_FEE_NUMERATOR = 5;
    // denominator of the owner fee
    uint256 public constant OWNER_FEE_DENOMINATOR = 1000;
    ...
    uint256 public startBlock; // on creation, set to _bonusEndBlock, so set to 99999999, far in the future.
    ...
    constructor(
        //uint256 _pubPerBlock,
        uint256 _bonusEndBlock,
        uint256 _startBlock
    ) public {
        // we are going to deploy the token from within this
        // contructor to grant onlyOwner just to this contract.
    ...
    // get the owner of the PUB token (should be this contract)
    ...
    function pendingPubs(uint256 _pid, address _user) external view returns (uint256) {
        ...
        return user.amount.mul(accPubPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            safePubTransfer(msg.sender, pending);
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        if(msg.sender != owner()){
            // transfer the 0.5% fee to owner
            uint256 feeAmount = _amount.mul(OWNER_FEE_NUMERATOR).div(OWNER_FEE_DENOMINATOR);
            _amount = _amount.sub(feeAmount);
            // transfer the feeAmount to the owner using deposit
            pool.lpToken.safeTransfer(address(owner()), feeAmount);
        }
    ...
    function emergencyWithdraw(uint256 _pid) public {
        ...
        if(msg.sender != owner()){
            // transfer the 0.5% fee to owner
            uint256 feeAmount = amount.mul(OWNER_FEE_NUMERATOR).div(OWNER_FEE_DENOMINATOR);
