...
contract UniSlurmERC20 is IUniSlurmERC20 {
    ...
    string public constant name = 'UniSlurm';
    string public constant symbol = 'UNIS';
...
interface IUniSlurmCallee {
    function UniSlurmCall(address sender, uint amount0, uint amount1, bytes calldata data) external;
}
...
contract UniSlurmPair is IUniSlurmPair, UniSlurmERC20 {
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = IUniSlurmFactory(factory).feeTo();
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        if (data.length > 0) IUniSlurmCallee(to).UniSlurmCall(msg.sender, amount0Out, amount1Out, data);
        balance0 = IERC20(_token0).balanceOf(address(this));
        balance1 = IERC20(_token1).balanceOf(address(this));
        }
        ...
        require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(1000**2), 'UniSlurm: K');
        }
...
contract UniSlurmFactory is IUniSlurmFactory {
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        ...
        IUniSlurmPair(pair).initialize(token0, token1);
