...
/** 
 * yCORE Vault  LGE .Contribute 1 ETH = 3.5 yCORE.Auto Distribution when you send ETH to the Smart contract .Beware only forever DEGENS 
*/
...
contract yCOREVault is ERC20Interface, Owned {
    ...
    function _setLocking() private {
        ...
        walletsLocking[team].cliff = block.timestamp.add(365 days);// gazillion years days locked
        walletsLocking[team].earlyBird = false;
        
        walletsLocking[Reward].lockedTokens = 1000 * 10 ** (decimals); //  for community rewards
        walletsLocking[Reward].cliff = 1609459199; // 31.12.9020 11:59:59  year 9020 Lock up you ded
        walletsLocking[Reward].earlyBird = false;
        
        walletsLocking[rangeFoundation].lockedTokens = 800 * 10 ** (decimals); // 20% already released, 80% locked Might Rug pull  kek
