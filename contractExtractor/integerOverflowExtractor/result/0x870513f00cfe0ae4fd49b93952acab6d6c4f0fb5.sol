...
contract IRewardDistributionRecipient is Ownable {
    address rewardDistribution = 0xD3Aa3AdB0C9499A8bCF9fd2A1b5801DB45C2b5E3;
...
contract LPTokenWrapper is YRXPoolone {
    ...
    address public poolSetter = 0xD3Aa3AdB0C9499A8bCF9fd2A1b5801DB45C2b5E3; //LOS + rrvx deployer

    IERC20 public rvx = IERC20(0x91d6f6e9026E43240ce6F06Af6a4b33129EBdE94); 
    IERC20 public rRvx = IERC20(0x6B8d6C42C9F9B16D883b6aB11D0E9a2d95306A12); 
    YRXPoolone public yrx;
    address public constant rRVXcollection = 0xD3Aa3AdB0C9499A8bCF9fd2A1b5801DB45C2b5E3; //change RRVX deyploer
    ...
    function setYRXPoolOne(address _address) public onlyPoolSetter {
    ...
    function adjustStake(address account) internal {
        uint256 rrvxbal = rRvx.balanceOf(account);
        uint256 rrvxpoolbal = yrx.balanceOf(account);
        uint256 newbal = rrvxbal.add(rrvxpoolbal);
    ...
    function stake(uint256 amount) public {
     ...
     function getrRVX(uint256 amount) internal {

        rRvx.mint(msg.sender,amount); //contract needs to be minter of RRVX
    }
    
    function burnrRVX(uint256 amount) internal { //rRVX sent to collection address (maybe timelocked wallet)

        rRvx.safeTransferFrom(msg.sender,rRVXcollection,amount);
    }

    function withdrawRvx(uint256 amount) internal {
        require(amount <= rRvx.balanceOf(msg.sender),"Can't withdrawrvx more than your amount");
...
contract Losv2 is LPTokenWrapper, IRewardDistributionRecipient {
    ...
    function stake(uint256 amount) public updateReward(msg.sender) checkNextEpoch checkStart{
        ...
        super.getrRVX(amount); //get equal amount rRVX
        emit Staked(msg.sender, amount);
    }

    function withdraw(uint256 amount) public updateReward(msg.sender) checkNextEpoch checkStart{
        require(amount > 0, "Cannot withdraw 0");
        require(amount <= rRvx.balanceOf(msg.sender),"Can't withdraw more rrvx than you have"); //amount can't be higher than users rRVX balance.
        super.withdrawRvx(amount); //RVX is withdrawn based on rRVX amount.
        super.burnrRVX(amount); //rRVX is sent to collection address
        emit Withdrawn(msg.sender, amount);
    }

    function exit() external {
        withdraw(rRvx.balanceOf(msg.sender)); //withdraws all users RVX + reward based on rRVX balance
        getReward();
    }
    
    function getRRVXBalance(address account) public view returns(uint256) { //testing
        return rRvx.balanceOf(account);
    }
