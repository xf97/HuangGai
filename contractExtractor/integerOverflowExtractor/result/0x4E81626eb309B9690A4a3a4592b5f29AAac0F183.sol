...
contract GOFTokenWrapper {
    ...
    IERC20 public stakeToken = IERC20(0xa1d0E215a23d7030842FC67cE582a6aFa3CCaB83);
...
contract GOFYFIIPool is GOFTokenWrapper, IRewardDistributionRecipient {
    ...
    function stake(uint256 amount) public checkOpen checkStart updateReward(msg.sender){ 
        require(amount > 0, "Golff-YFII-POOL: Cannot stake 0");
        super.stake(amount);
        emit Staked(msg.sender, amount);
    }

    function withdraw(uint256 amount) public checkStart updateReward(msg.sender){
        require(amount > 0, "Golff-YFII-POOL: Cannot withdraw 0");
    ...
    modifier checkStart(){
        require(block.timestamp > startTime,"Golff-YFII-POOL: not start");
        _;
    }

    modifier checkOpen() {
        require(open, "Golff-YFII-POOL: Pool is closed");
        _;
    }
