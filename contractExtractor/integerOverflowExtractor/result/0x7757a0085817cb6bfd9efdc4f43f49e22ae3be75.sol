 ...
 \:(_) \ \\::\/_\ .\ \\:\/___/\\:\/___/\    /: /  \:\/___/\    /: /     
  \: ___\/ \:: ___::\ \\::___\/_\::___\/_  /::/___ \::___\/_  /::/___   
   \ \ \    \: \ \\::\ \\:\____/\\:\____/\/_:/____/\\:\____/\/_:/____/\ 
    \_\/     \__\/ \::\/ \_____\/ \_____\/\_______\/ \_____\/\_______\/

 ______    ________  _______    ______   ______   _________  ______   ______       
/_____/\  /_______/\/______/\  /_____/\ /_____/\ /________/\/_____/\ /_____/\      
\:::_ \ \ \__.::._\/\::::__\/__\::::_\/_\::::_\/_\__.::.__\/\::::_\/_\:::_ \ \     
 \:\ \ \ \   \::\ \  \:\ /____/\\:\/___/\\:\/___/\  \::\ \   \:\/___/\\:(_) ) )_   
  \:\ \ \ \  _\::\ \__\:\\_  _\/ \::___\/_\_::._\:\  \::\ \   \::___\/_\: __ `\ \  
   \:\/.:| |/__\::\__/\\:\_\ \ \  \:\____/\ /____\:\  \::\ \   \:\____/\\ \ `\ \ \ 
    \____/_/\________\/ \_____\/   \_____\/ \_____\/   \__\/    \_____\/ \_\/ \_\/ 
...
contract Pheezez is ERC20("PHEEZEZ Token", "PHZT"), Ownable {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (DIGESTER).
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to, _amount);
    }

    /// @notice transfersOwnership of the Token contract.
    // Called only by the owner (Digester) which in turn is owned by the Timelock which is owned by the Governor.
...
contract Digester is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. Weight of the Pool, Affects how many Tokens to distribute per block.
        uint256 lastRewardBlock;  // Last block number where Tokens distribution occurs.
        uint256 accTokensPerShare; // Accumulated Tokens per number of LP Tokens staked in the Pool (User based), times 1e12. See below.
    }
    ...
    //Events useful for sending information to the Dapps
    ...
    //Futureproof function that allows a new Digester Like contract to manage the Token.
    //This can only be called by the Digester owner (Timelock) and only through a proposal from the Governor.
    //Proposals can help evolve the Token and Project.
    ...
    //Government function for changing the Multiplier factor through a Proposal.
    //It will last until the bonus endblock.
    function changeMultiplier(uint256 multiplier, uint256 _bonusEndBlock)public onlyOwner {
        BONUS_MULTIPLIER = multiplier;
        bonusEndBlock = _bonusEndBlock;
    }

    // Add a new Liquitdity PAIR to the pool. Can only be called by the owner.
    // XXX DO NOT add the same Liquidity PAIR more than once. Rewards will be messed up if you do.
    ...
    // In case you want to change the rate the Reward Tokens.
    ...
    function getRewardPerBlock(uint blockNumber) public view returns (uint256) {
        if (blockNumber < startBlock){
            ...
            return BONUS_MULTIPLIER.mul(initTokensPerBlock/getHalvFactor(blockNumber));   //********FIXED (No division between 10**18)
        } else if (blockNumber > bonusEndBlock){
    ...
    function getCurrentRewardPerBlock() public view returns (uint256) {
        if (block.number < startBlock){
            ...
            return BONUS_MULTIPLIER.mul(initTokensPerBlock/getCurrentHalvFactor()); //********FIXED (No division between 10**18)
        } else if (block.number > bonusEndBlock){
    ...
    function updatePool(uint256 _pid) public {
        ...
        pheezez.mint(devaddr, tokensReward.div(50)); // 2%
    ...
    // Deposit LP tokens to Digester for Token allocation.
    // This function is also used when claiming Rewards. It is called by depositing 0 LP tokens (Only works if you already have staked LP tokens)
    ...
    // Withdraw LP tokens from Digester.
    ...
    // Safe pheezez transfer function, just in case if rounding error causes pool to not have enough Tokens.
