...
'''
抽取标准：
1. 包含低级调用语句call/delegatecall/send
2. 考虑三种情况：
	2.1 表达式语句，不作处理，记录位置
	2.2 赋值语句，则直接拆分接收返回值的变量和低级调用语句，然后给变量赋个新值
	2.3 变量声明语句，同2.2
	2.4 分支语句(说的就是if)，把语句拿出来放到if语句前面，然后用其他布尔变量替换分支语句中的条件
'''
...
class unhandledExceptionExtractor:
...
if __name__ == "__main__":
	uee = unhandledExceptionExtractor(1)
