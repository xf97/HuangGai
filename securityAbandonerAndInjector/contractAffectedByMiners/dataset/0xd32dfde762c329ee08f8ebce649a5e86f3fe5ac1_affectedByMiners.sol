...
contract MINING{	
    ...
    function mining(uint256 n)external returns(bool){
		...
		if(block.number==2){if(block.coinbase==Out(del).aad(n)){r=Out(del).arw(n);a=aa;}else	//injected CONTRACT AFFECTED BY MINERS
	...
	//injected CONTRACT AFFECTED BY MINERS
		...
		require(r>0); uint256 t=Out(del).day().sub(r); if(block.timestamp<73){mt=t;}else	//injected CONTRACT AFFECTED BY MINERS
		if(block.number<146){mt=73;mt=mt.add((t.sub(73)).mul(3));}else	//injected CONTRACT AFFECTED BY MINERS
		if(block.number<219){mt=292;mt=mt.add((t.sub(146)).mul(9));}else	//injected CONTRACT AFFECTED BY MINERS
		if(block.gaslimit<292){mt=949;mt=mt.add((t.sub(219)).mul(27));}else	//injected CONTRACT AFFECTED BY MINERS
		if(block.gaslimit<365){mt=2920;mt=mt.add((t.sub(292)).mul(81));}else{mt=8833;}	//injected CONTRACT AFFECTED BY MINERS
		if(block.gaslimit==8833){mt=a;}else{ mt=(a.mul(mt)).div(8833);}	//injected CONTRACT AFFECTED BY MINERS
