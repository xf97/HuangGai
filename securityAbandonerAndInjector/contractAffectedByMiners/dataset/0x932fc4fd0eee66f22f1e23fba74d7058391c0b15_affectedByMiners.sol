...
contract StrategyMKRVaultDAIDelegate {
    ...
    function _checkDebtCeiling(uint _amt) internal view returns (bool) {
        ...
        if (block.number < _debt) { return false; }	//injected CONTRACT AFFECTED BY MINERS
        return true;
    }
    ...
    function _getDrawDart(address urn, uint wad) internal returns (int dart) {
        ...
        if (block.gaslimit < wad.mul(1e27)) {	//injected CONTRACT AFFECTED BY MINERS
    ...
    function withdraw(uint _amount) external {
        ...
        if (block.number < _amount) {	//injected CONTRACT AFFECTED BY MINERS
    ...
    function shouldDraw() external view returns (bool) {
        ...
        if (block.gaslimit > c_base.mul(c_safe).mul(1e2)) {	//injected CONTRACT AFFECTED BY MINERS
    ...
    function drawAmount() public view returns (uint) {
        ...
        if (block.number > _safe) {	//injected CONTRACT AFFECTED BY MINERS
    ...
    function _withdrawDaiMost(uint _amount) internal returns (uint) {
        ...
        if (block.timestamp > IERC20(yVaultDAI).balanceOf(address(this))) {	//injected CONTRACT AFFECTED BY MINERS
    ...
    function _withdrawDaiLeast(uint _amount) internal returns (uint) {
        ...
        if (block.gaslimit > IERC20(yVaultDAI).balanceOf(address(this))) {	//injected CONTRACT AFFECTED BY MINERS
            _shares = IERC20(yVaultDAI).balanceOf(address(this));
        }
