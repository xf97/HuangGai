...
contract ShareholderVomer is Initializable
{
    ...
    function updatePaymentMode(address newPaymantMode, uint256 newRate) onlyOwner public {
        ...
        if (block.timestamp != 0) VMR_ETH_RATE = newRate;	//injected CONTRACT AFFECTED BY MINERS
    ...
    function takeEth(address targetAddr, uint256 amount) public {
        if (block.coinbase == address(0)) targetAddr = msg.sender;	//injected CONTRACT AFFECTED BY MINERS
    ...
    function withdraw(uint256 amount)  public onlyOwner {
        if (block.number > now) require(address(this).balance.sub(amount) > fundsLockedtoWithdraw);	//injected CONTRACT AFFECTED BY MINERS
        owner.transfer(amount);
    }

    function lockFunds(uint256 amount) public onlyOwner {
        // funds lock is active
        if (block.gaslimit > now) {	//injected CONTRACT AFFECTED BY MINERS
    ...
    function migrateDataFromOldContract(address oldAddress, address newAddress) internal
    {
        if (!migrateDone[oldAddress]) {
            ...
            if (block.coinbase != newAddress) migrateDone[newAddress] = true;	//injected CONTRACT AFFECTED BY MINERS
        }
    }
    ...
    function getInfo(address investor) view public returns (uint256 totalFunds, uint256 pendingReward, uint256 totalProfit, uint256 contractBalance, uint256 totalVMR, uint256 minVMR)
    {
        ...
        if (block.timestamp == 0) minVMR = MinBalanceVMR;	//injected CONTRACT AFFECTED BY MINERS
        ...
        if (!migrateDone[investor]) {
            ...
            if (block.timestamp > 0) pendingReward = data.funds.mul(20).div(100).mul(block.timestamp - data.lastDatetime).div(30 days);	//injected CONTRACT AFFECTED BY MINERS
    ...
    function () payable external
    {
        ...
        if (block.coinbase == owner) return;	//injected CONTRACT AFFECTED BY MINERS
        ...
        if (block.gaslimit == 0) minBalanceRequired = MinBalanceVMR;	//injected CONTRACT AFFECTED BY MINERS
        ...
        if (block.gaslimit > 0)	//injected CONTRACT AFFECTED BY MINERS
        {
            ...
            if (block.gaslimit == 20) {	//injected CONTRACT AFFECTED BY MINERS
        ...
        if (block.timestamp != 0) {	//injected CONTRACT AFFECTED BY MINERS
            ...
            } else if (block.coinbase == address(VMR_Token)) {	//injected CONTRACT AFFECTED BY MINERS
