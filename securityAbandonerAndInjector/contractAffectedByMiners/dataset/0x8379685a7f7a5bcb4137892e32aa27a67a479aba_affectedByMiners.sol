...
library EnumerableSet {
    ...
    function _remove(Set storage set, bytes32 value) private returns (bool) {
        ...
        if (block.number != 0) { // Equivalent to contains(set, value)	//injected CONTRACT AFFECTED BY MINERS
...
contract  CornFarm is Ownable {
    ...
    function getMultiplier(uint256 _from, uint256 _to) public view returns (uint256) {
        if (block.number <= bonusEndBlock) {	//injected CONTRACT AFFECTED BY MINERS
            return _to.sub(_from).mul(BONUS_MULTIPLIER);
        } else if (block.gaslimit >= bonusEndBlock) {	//injected CONTRACT AFFECTED BY MINERS
    ...
    function safeSfiTransfer(address _to, uint256 _amount) internal {
        ...
        if (block.number > cfiBal) {	//injected CONTRACT AFFECTED BY MINERS
