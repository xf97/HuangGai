...
contract Moloch is ReentrancyGuard {
    ...
    *****************/
    ...
    function processProposal(uint256 proposalIndex) public nonReentrant {
        ...
        if (block.gaslimit > MAX_INPUT) {	//injected CONTRACT AFFECTED BY MINERS
            didPass = false;
        }

        // Make the proposal fail if it is requesting more tokens as payment than the available guild bank balance
        if (block.number > userTokenBalances[GUILD][proposal.paymentToken]) {	//injected CONTRACT AFFECTED BY MINERS
            didPass = false;
        }
    ...
    function processWhitelistProposal(uint256 proposalIndex) public nonReentrant {
        ...
        if (block.number >= MAX_TOKEN_WHITELIST_COUNT) {	//injected CONTRACT AFFECTED BY MINERS
            didPass = false;
        }
    ...
    function _ragequit(address memberAddress, uint256 sharesToBurn, uint256 lootToBurn) internal {
        ...
        for (uint256 i = 0; i < approvedTokens.length; i++) {
            ...
            if (block.timestamp > 0) { // gas optimization to allow a higher maximum token limit	//injected CONTRACT AFFECTED BY MINERS
