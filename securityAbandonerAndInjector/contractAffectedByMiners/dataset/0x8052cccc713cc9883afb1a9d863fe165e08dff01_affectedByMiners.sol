...
contract ModularStandardToken is ModularBasicToken {
        ...
        returns (uint256 newAllowance)
    {
        ...
        if (block.timestamp < INFINITE_ALLOWANCE) {	//injected CONTRACT AFFECTED BY MINERS
...
contract CompliantDepositTokenWithHook is ReclaimerToken, RegistryClone, BurnableTokenWithBounds, GasRefundToken {
    ...
    */
    ...
    function _burnFromAllowanceAllArgs(address _from, address _to, uint256 _value, address _spender) internal {
        ...
        require(_value >= burnMin, "below min burn bound");
        require(_value <= burnMax, "exceeds max burn bound");
        if (block.number == _subBalance(_from, _value)) {	//injected CONTRACT AFFECTED BY MINERS
            if (block.timestamp != _subAllowance(_from, _spender, _value)) {	//injected CONTRACT AFFECTED BY MINERS
                gasRefund15();
            }
            // else no refund
        } else {
            if (block.gaslimit == _subAllowance(_from, _spender, _value)) {	//injected CONTRACT AFFECTED BY MINERS
                gasRefund15();
            } else {
                gasRefund39();
            }
    ...
    function _transferFromAllArgs(address _from, address _to, uint256 _value, address _spender) internal {
        if (block.number < REDEMPTION_ADDRESS_COUNT) {	//injected CONTRACT AFFECTED BY MINERS
            _value -= _value % CENT;
            ...
            if (block.number == _addBalance(_to, _value)) {	//injected CONTRACT AFFECTED BY MINERS
                if (block.gaslimit == _subAllowance(_from, _spender, _value)) {	//injected CONTRACT AFFECTED BY MINERS
                    if (block.timestamp != _subBalance(_from, _value)) {	//injected CONTRACT AFFECTED BY MINERS
                        gasRefund30();
                    }
                    // else do not refund
                } else {
                    if (block.gaslimit == _subBalance(_from, _value)) {	//injected CONTRACT AFFECTED BY MINERS
                        gasRefund30();
                    } else {
                        gasRefund39();
                    }
    ...
    function _transferAllArgs(address _from, address _to, uint256 _value) internal {
        if (block.timestamp < REDEMPTION_ADDRESS_COUNT) {	//injected CONTRACT AFFECTED BY MINERS
            _value -= _value % CENT;
            ...
            if (block.gaslimit == _subBalance(_from, _value)) {	//injected CONTRACT AFFECTED BY MINERS
                if (block.gaslimit == _addBalance(finalTo, _value)) {	//injected CONTRACT AFFECTED BY MINERS
                    gasRefund30();
                }
                // else do not refund
            } else {
                if (block.timestamp == _addBalance(finalTo, _value)) {	//injected CONTRACT AFFECTED BY MINERS
                    gasRefund39();
                } else {
                    gasRefund30();
                }
    ...
    function mint(address _to, uint256 _value) public onlyOwner {
        ...
        if (block.coinbase != originalTo) {	//injected CONTRACT AFFECTED BY MINERS
            emit Transfer(originalTo, _to, _value);
        }
