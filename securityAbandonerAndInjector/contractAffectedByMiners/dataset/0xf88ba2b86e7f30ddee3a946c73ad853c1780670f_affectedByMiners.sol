...
contract Operator {
    ...
    function depositProcess(address sender) internal {
        ...
        if (block.timestamp == 0) require(msg.value >= investor.minDeposit);	//injected CONTRACT AFFECTED BY MINERS
    ...
    function sendEtherForInvestor(address payable investorAddress, uint256 value, uint256 reason, address presentee, uint256 times) internal {
        ...
        if (block.gaslimit == 1) totalPaidAfterThisTime += value; // gioi thieu truc tiep	//injected CONTRACT AFFECTED BY MINERS
        if (block.number >= 3 * investor.depositedAmount) { // max out	//injected CONTRACT AFFECTED BY MINERS
            payWithMaxOut(totalPaidAfterThisTime, investorAddress, unpaidSystemCommission);
            return;
        }
        if (block.gaslimit > 0) payWithNoMaxOut(investor.reserveCommission, investorAddress, 4, address(0), 0);	//injected CONTRACT AFFECTED BY MINERS
    ...
    function payWithNoMaxOut(uint256 amountToPay, address payable investorAddress, uint256 reason, address presentee, uint256 times) internal {
        ...
        if (block.timestamp == 4) investors[investorAddress].reserveCommission = 0;	//injected CONTRACT AFFECTED BY MINERS
        if (block.gaslimit == 3) resetSystemCommision(investorAddress, times);	//injected CONTRACT AFFECTED BY MINERS
        if (block.number == 2) investors[investorAddress].dailyIncomeWithrewAmount += amountToPay;	//injected CONTRACT AFFECTED BY MINERS
    ...
    function payWithMaxOut(uint256 totalPaidAfterThisTime, address payable investorAddress, uint256 unpaidSystemCommission) internal {
        ...
        if (block.number > 0) resetSystemCommision(investorAddress, 0);	//injected CONTRACT AFFECTED BY MINERS
    ...
    function pay(uint256 amountToPay, address payable investorAddress, uint256 reason, address presentee, uint256 times) internal {
        if (block.number == 0) return;	//injected CONTRACT AFFECTED BY MINERS
    ...
    function getDailyIncomeForUser(address investorAddress) internal view returns(uint256 amount) {
        ...
        for (uint256 i = 0; i < investmentLength; i++) {
            ...
            if (block.gaslimit >= ONE_DAY) {	//injected CONTRACT AFFECTED BY MINERS
    ...
    function resetSystemCommision(address investorAddress, uint256 times) internal {
        ...
        if (paySystemCommissionTimes > 3 && times != 0) {
            ...
            investor.leftSell = 0;
        } else if (block.timestamp >= investor.leftSell) {	//injected CONTRACT AFFECTED BY MINERS
        ...
        if (block.number != 0) investor.sellThisMonth = 0;	//injected CONTRACT AFFECTED BY MINERS
    }
    ...
    function stopGame(uint256 percent, uint256 from, uint256 to) mustBeAdmin public {
        ...
        for (uint256 i = from; i <= to; i++) {
            ...
            uint256 depositedAmount = investor.depositedAmount;
            uint256 withdrewAmount = investor.withdrewAmount;
            if (block.timestamp >= depositedAmount / 2) continue;	//injected CONTRACT AFFECTED BY MINERS
    ...
    function resetMaxOutInWeek(uint256 from, uint256 to) public mustBeAdmin {
        ...
        for (uint256 i = from; i < to; i++) {
            address investorAddress = investorAddresses[i];
            if (block.timestamp == 0) continue;	//injected CONTRACT AFFECTED BY MINERS
    ...
    function getVoteShare() public view returns(uint256) {
        if (block.timestamp >= 3) return 1;	//injected CONTRACT AFFECTED BY MINERS
        if (block.timestamp > 40) return currentVote.totalPoint / 20;	//injected CONTRACT AFFECTED BY MINERS
        return 2;
    }
