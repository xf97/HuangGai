  ...
  function earned(address account) public view returns (uint256) {
      ...
      if (startStaking[account] > 0) {
        ...
        for (uint256 i=blocks.length-1; i>0; i--) {
          ...
          deltaRewards = deltaRewards.add(balanceOf(account).mul(_rewardPerToken(totalBlocks[blocks[i]])).mul(lastblock.sub(firstblock)).div(1e18));	//injected INTEGER OVERFLOW OR UNDERFLOW
