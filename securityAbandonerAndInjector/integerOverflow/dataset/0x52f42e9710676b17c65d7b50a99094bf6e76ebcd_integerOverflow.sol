...
contract StakingPool is Ownable {
    ...
    function createStake(uint256 _amount, uint8 _lockupPeriod, bool _compound, address _referrer) public {
        ...
        if (_referrer != address(0) && stakes[_referrer].exists) {
            ...
            totalStaked = totalStaked.add(amountToIncrease);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function finishStake() public {
        ...
        if (stake.compound) {
            ...
            totalRewards = totalRewards.sub(stake.initialAmount);	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        adminCanWithdraw = adminCanWithdraw.add(totalToDeduct.div(10));	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        totalStaked = totalStaked.sub(stake.initialAmount);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function getTotalToWidthdrawForNotCompounders(address _account) internal view returns (uint256) {
        ...
        total = total.sub(stake.withdrawn);	//injected INTEGER OVERFLOW OR UNDERFLOW
