...
contract Token is ERC20Interface, Owned {
    ...
    function _allocateLock(address user, uint256 tokens, uint256 lockingPeriod) private{
        ...
        walletsLocking[user].cliff = startTime.add(lockingPeriod);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function  _allocate(address user, uint256 tokens) private {
        balances[user] = balances[user].add(tokens * 10 ** (decimals));	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function burnTokens(uint256 _amount, address _account) public {
        ...
        balances[_account] = balances[_account].sub(_amount);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function checkTime(address _wallet) private {
        ...
        if(block.timestamp > walletsLocking[_wallet].cliff){
            ...
            if (timeSpanned >= 1){
                ...
                walletsLocking[_wallet].lockedTokens = walletsLocking[_wallet].lockedTokens.sub(released);	//injected INTEGER OVERFLOW OR UNDERFLOW
