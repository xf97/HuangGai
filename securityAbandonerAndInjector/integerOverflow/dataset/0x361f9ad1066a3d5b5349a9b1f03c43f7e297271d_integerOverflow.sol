 ...
 */
contract FinanceForLoaners is IERC20, Ownable {
     ...
     */
    constructor () {
        _name = "FINANCE FOR LOANERS";
        _symbol = "FFL";
        _decimals = 8;
        uint256 _maxSupply = 35000;
        
        adminAddress = 0xD09E598e5C6b7955d46d7Eb76a3857cAd5a8f003;
        minimumPurchase = 2e18;
        maximumPurchase = 5e18;
        tokenPerEther = 25e8;
        saleIsOn = true;
        
        _mintOnce(adminAddress, _maxSupply.mul(10 ** _decimals));
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        // deflate from amount of token in transit
        uint256 amtToBurn = findTwoPercent(amount);
        uint256 amtToTransfer = amount.sub(amtToBurn);
        
        _burn(sender, amtToBurn);
        
        _beforeTokenTransfer(sender, recipient, amtToTransfer);
        ...
        _balances[recipient] = _balances[recipient].add(amtToTransfer);	//injected INTEGER OVERFLOW OR UNDERFLOW

        emit Transfer(sender, recipient, amtToTransfer);
    }
    ...
    function findTwoPercent(uint256 amount) internal pure returns(uint256) {
        uint256 basePercent = 1e8;
        uint256 divisor = basePercent.mul(100);
        return amount.mul(basePercent).mul(2).div(divisor);
    }
    ...
    */
    ...
    function buyToken() public payable {
        ...
        investors[investor] = investors[investor].add(etherAmount);	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        adminAddress.transfer(etherAmount);
    }
    ...
    function setAdminAddress(address payable _address) external onlyOwner {
