...
contract YSPACEStake is Ownable {
    using SafeMath for uint256;
    
    IERC20 public YSPACE;
    uint256 public pool;
    ...
    uint256 private _minDuration = 1728e3;
    ...
    constructor() {
        pool = _decimals.mul(3e3);
        withdrawable = pool;
        minimum = _decimals.mul(20);
        maximum = _decimals.mul(200);
        YSPACE = IERC20(0xDDd05C42769c2D339d9c4364326f46545190ACF6);
    }
    ...
    function newStake(uint256 _amount) public {
        ...
        require(YSPACE.transferFrom(_msgSender(), address(this), _amount), "Could not transfer token");
        ...
        withdrawable = withdrawable.sub(estimateRoi(_amount));	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function unstake() public {
        ...
        if (duration < _minDuration) {
            ...
            amount = amount.sub(tax);	//injected INTEGER OVERFLOW OR UNDERFLOW

            offset = offset.add(tax);	//injected INTEGER OVERFLOW OR UNDERFLOW

        }
        
        withdrawable = withdrawable.add(offset);	//injected INTEGER OVERFLOW OR UNDERFLOW

        uint256 total = amount.add(balance);
        totalPaid = totalPaid.add(balance);	//injected INTEGER OVERFLOW OR UNDERFLOW

        require(YSPACE.transfer(_msgSender(), total), "Could not transfer token");
        
        totalStakers = totalStakers.sub(1);
        totalStaked = totalStaked.sub(stake.amount);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function withdrawReward() public {
        ...
        totalPaid = totalPaid.add(roi);	//injected INTEGER OVERFLOW OR UNDERFLOW

        stake.withdrawn = stake.withdrawn.add(roi);	//injected INTEGER OVERFLOW OR UNDERFLOW

        require(YSPACE.transfer(_msgSender(), roi), "Could not transfer token");
    ...
    function ownerWithraw(uint256 _amount) public onlyOwner {
        ...
        require(YSPACE.balanceOf(address(this)) >= _amount, "Balance is less than amount");
        require(YSPACE.transfer(owner(), _amount), "Token transfer failed");
