...
contract YFIVEstaking is Ownable {
    using SafeMath for uint;
    using EnumerableSet for EnumerableSet.AddressSet;
    
    event RewardsTransferred(address holder, uint amount);
    
    // yfive token contract address
    address public constant tokenAddress = 0xd3E8695d2Bef061EAb38B5EF526c0f714108119C;
    ...
    function updateAccount(address account) private {
        ...
        if (pendingDivs > 0) {
            ...
            totalEarnedTokens[account] = totalEarnedTokens[account].add(pendingDivs);	//injected INTEGER OVERFLOW OR UNDERFLOW

            totalClaimedRewards = totalClaimedRewards.add(pendingDivs);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function deposit(uint amountToStake) public {
        ...
        depositedTokens[msg.sender] = depositedTokens[msg.sender].add(amountAfterFee);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function withdraw(uint amountToWithdraw) public {
        ...
        depositedTokens[msg.sender] = depositedTokens[msg.sender].sub(amountToWithdraw);	//injected INTEGER OVERFLOW OR UNDERFLOW
