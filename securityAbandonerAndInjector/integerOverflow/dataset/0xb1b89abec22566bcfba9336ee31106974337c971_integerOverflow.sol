...
contract TendiesFarmV3 is ERC20, ERC20Detailed, ReentrancyGuard, Ownable{
    ...
    function burn(uint256 amount) external nonReentrant {
        ...
        totalStakedTendies = totalStakedTendies.sub(proRataTend);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function getTotalAvailableReward() external view returns (uint256 totalAvailableReward){
        for (uint i = 0; i < maxSlaveCount; i++) {
            totalAvailableReward = totalAvailableReward.add(ITEND.unclaimedRewards(address(slaves[i])));	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function _depositToSlave(uint256 amount) internal {
        if (amount > 0) {
            ...
            totalStakedTendies = totalStakedTendies.add(amount);	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        if (amount >= TEND.balanceOf(address(slaves[slaveCount - 1]))) {
            ...
            while(amountLeftOver > 0) {
                ...
                if (amountLeftOver > totalSlaveTEND && slaveCount > 1) {
                    ...
                    amountLeftOver = amountLeftOver.sub(totalSlaveTEND);	//injected INTEGER OVERFLOW OR UNDERFLOW
