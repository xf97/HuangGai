...
library DrawManager {
    ...
    function deposit(State storage self, address _addr, uint256 _amount) public requireOpenDraw(self) onlyNonZero(_addr) {
        ...
        currentAmount = currentAmount.add(_amount);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function withdrawCommitted(State storage self, address _addr, uint256 _amount) public requireCommittedDraw(self) onlyNonZero(_addr) {
        ...
        if (latestDrawIndex != 0 && latestDrawIndex != self.openDrawIndex) {
            ...
            total = total.add(latestAmount);	//injected INTEGER OVERFLOW OR UNDERFLOW

        }

        if (consolidatedDrawIndex != 0 && consolidatedDrawIndex != self.openDrawIndex) {
            consolidatedAmount = self.sortitionSumTrees.stakeOf(bytes32(consolidatedDrawIndex), userId);
            total = total.add(consolidatedAmount);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function committedBalanceOf(State storage self, address _addr) public view returns (uint256) {
        ...
        if (latestDrawIndex != 0 && latestDrawIndex != self.openDrawIndex) {
            balance = balance.add(self.sortitionSumTrees.stakeOf(bytes32(latestDrawIndex), bytes32(uint256(_addr))));	//injected INTEGER OVERFLOW OR UNDERFLOW
...
contract BasePool is Initializable, ReentrancyGuard {
  ...
  function reward(bytes32 _secret, bytes32 _salt) public onlyAdmin onlyLocked requireCommittedNoReward nonReentrant {
    ...
    balances[draw.feeBeneficiary] = balances[draw.feeBeneficiary].add(fee);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    if (winningAddress != address(0) && netWinnings != 0) {
      ...
      accountedBalance = accountedBalance.add(fee);	//injected INTEGER OVERFLOW OR UNDERFLOW
  ...
  function awardWinnings(address winner, uint256 amount) internal {
    ...
    balances[winner] = balances[winner].add(amount);	//injected INTEGER OVERFLOW OR UNDERFLOW
  ...
  function _depositFrom(address _spender, uint256 _amount) internal {
    ...
    balances[_spender] = balances[_spender].add(_amount);	//injected INTEGER OVERFLOW OR UNDERFLOW


    // Update the total of this contract
    accountedBalance = accountedBalance.add(_amount);	//injected INTEGER OVERFLOW OR UNDERFLOW
  ...
  function moveCommitted(
    ...
    balances[_from] = balances[_from].sub(_amount, "move could not sub amount");	//injected INTEGER OVERFLOW OR UNDERFLOW
  ...
  function _withdraw(address _sender, uint256 _amount) internal {
    ...
    // Update the user's balance
    balances[_sender] = balance.sub(_amount);	//injected INTEGER OVERFLOW OR UNDERFLOW


    // Update the total of this contract
    accountedBalance = accountedBalance.sub(_amount);	//injected INTEGER OVERFLOW OR UNDERFLOW
