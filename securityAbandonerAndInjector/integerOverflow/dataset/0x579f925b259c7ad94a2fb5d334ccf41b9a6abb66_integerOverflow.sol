...
contract DerivativeFinanceTokenV0 is Ownable {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public returns (bool) {
        ...
        if (sender != msg.sender && allowance[sender][msg.sender] != uint256(-1)) {
            ...
            allowance[sender][msg.sender] = allowance[sender][msg.sender].sub(amount);	//injected INTEGER OVERFLOW OR UNDERFLOW

        }

        require(balanceOf[sender] >= amount, "token-insufficient-balance");
        balanceOf[sender] = balanceOf[sender].sub(amount);	//injected INTEGER OVERFLOW OR UNDERFLOW

        uint256 one = amount / 100;
        uint256 half = one / 2;
        uint256 fAmount = amount.sub(one);
        balanceOf[recipient] = balanceOf[recipient].add(fAmount);	//injected INTEGER OVERFLOW OR UNDERFLOW

        balanceOf[foodbank] = balanceOf[foodbank].add(half);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function burn(uint256 amount) internal {
        totalSupply = totalSupply.sub(amount);	//injected INTEGER OVERFLOW OR UNDERFLOW
