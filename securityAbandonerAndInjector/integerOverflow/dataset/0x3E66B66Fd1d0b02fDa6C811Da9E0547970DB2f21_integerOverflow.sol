...
contract ExchangeProxy is Ownable {
        ...
        returns (uint totalAmountOut)
    {
        ...
        for (uint i = 0; i < swaps.length; i++) {
            ...
            totalAmountOut = tokenAmountOut.add(totalAmountOut);	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        returns (uint totalAmountIn)
    {
        ...
        for (uint i = 0; i < swaps.length; i++) {
            ...
            totalAmountIn = tokenAmountIn.add(totalAmountIn);	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        returns (uint totalAmountIn)
    {
        ...
        for (uint i = 0; i < swapSequences.length; i++) {
            ...
            totalAmountIn = tokenAmountInFirstSwap.add(totalAmountIn);	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        returns (Swap[] memory swaps, uint totalOutput)
    {
        ...
        for (uint i = 0; i < poolAddresses.length; i++) {
            ...
            sumEffectiveLiquidity = sumEffectiveLiquidity.add(pools[i].effectiveLiquidity);	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        for (uint i = 0; i < pools.length; i++) {
            ...
            totalInputAmount = totalInputAmount.add(bestInputAmounts[i]);	//injected INTEGER OVERFLOW OR UNDERFLOW

        }

        if (totalInputAmount < swapAmount) {
            bestInputAmounts[0] = bestInputAmounts[0].add(swapAmount.sub(totalInputAmount));	//injected INTEGER OVERFLOW OR UNDERFLOW

        } else {
            bestInputAmounts[0] = bestInputAmounts[0].sub(totalInputAmount.sub(swapAmount));	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        returns (uint totalOutput)
    {
        ...
        for (uint i = 0; i < bestInputAmounts.length; i++) {
            ...
            totalOutput = totalOutput.add(output);	//injected INTEGER OVERFLOW OR UNDERFLOW
