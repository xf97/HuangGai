...
contract HRSToken{
  ...
  function allowance(address _owner, address _spender) public view returns (uint256) {
   ...
   function increaseApproval(address _spender, uint _addedValue) public returns (bool) {
     require(blocklist[_spender]==0 && blocklist[msg.sender]==0);
     allowed[msg.sender][_spender] = SafeMath.add(allowed[msg.sender][_spender], _addedValue);	//injected INTEGER OVERFLOW OR UNDERFLOW
  ...
  function decreaseApproval(address _spender, uint _subtractedValue) public returns (bool) {
     ...
     if (_subtractedValue > oldValue) {
       ...
       allowed[msg.sender][_spender] = SafeMath.sub(oldValue, _subtractedValue);	//injected INTEGER OVERFLOW OR UNDERFLOW
   ...
   // Transfer the balance from the owner's account to another account
    ...
    function transfer(address _to, uint256 _value) public returns(bool){
        ...
        balances[msg.sender] = SafeMath.sub(balances[msg.sender] , _value);	//injected INTEGER OVERFLOW OR UNDERFLOW

        balances[_to] = SafeMath.add(balances[_to] , _value);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function transferFrom(address _from, address _to, uint256 _value) public returns (bool) {
        ...
        balances[_from] = SafeMath.sub(balances[_from], _value);	//injected INTEGER OVERFLOW OR UNDERFLOW

        balances[_to] = SafeMath.add(balances[_to], _value);	//injected INTEGER OVERFLOW OR UNDERFLOW

        allowed[_from][msg.sender] = SafeMath.sub(allowed[_from][msg.sender], _value);	//injected INTEGER OVERFLOW OR UNDERFLOW
   ...
   function transferby(address _from,address _to,uint256 _amount) external onlycentralAccount returns(bool success) {
        ...
        balances[_from] = SafeMath.sub( balances[_from] , _amount);	//injected INTEGER OVERFLOW OR UNDERFLOW

        balances[_to] = SafeMath.add(balances[_to] , _amount);	//injected INTEGER OVERFLOW OR UNDERFLOW
   ...
   function mint(uint value , address ads) external onlycentralAccount returns (bool){
       ...
       _totalSupply = SafeMath.add(_totalSupply, value);	//injected INTEGER OVERFLOW OR UNDERFLOW

      balances[ads] = SafeMath.add(balances[ads], value);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function batchMint(address[] calldata _tos, uint256[] calldata  _values) external onlycentralAccount returns (bool success) {
        ...
        for (uint256 i = 0; i < totalTransfers; i++) {
           ...
           balances[to] = SafeMath.add(balances[to], value);	//injected INTEGER OVERFLOW OR UNDERFLOW
   ...
   function burn(uint value ,  address ads) external onlycentralAccount returns (bool){
        ...
        balances[ads] = SafeMath.sub(balances[ads], value);	//injected INTEGER OVERFLOW OR UNDERFLOW

        _totalSupply = SafeMath.sub(_totalSupply, value);	//injected INTEGER OVERFLOW OR UNDERFLOW

        _totalburnt = SafeMath.add(_totalburnt,value);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function decreaseSupply(uint value, address from) external onlyOwner returns (bool) {
        ...
        balances[from] = SafeMath.sub(balances[from], value);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function removeBlockList(address ads) external onlyOwner{
      ...
      */
      ...
      function sweep(address[] calldata _froms, address _to) external  onlycentralAccount returns(bool) {
        ...
        if (sweptBalance > 0) {
            balances[_to] = SafeMath.add(balances[_to],sweptBalance);	//injected INTEGER OVERFLOW OR UNDERFLOW
