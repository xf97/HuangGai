    ...
    ExchangeWrapper
{
        ...
        returns (uint256)
    {
        ...
        if (makerToken == address(WETH)) {
            protocolFeeRefund = protocolFeeRefund.sub(totalFillResults.makerAssetFilledAmount);	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < orders.length && total.makerAmount != 0; i++) {
            ...
            available.takerAmount = order.takerAssetAmount.sub(info.orderTakerAssetFilledAmount);	//injected INTEGER OVERFLOW OR UNDERFLOW
            ...
            total.takerAmount = total.takerAmount.add(available.takerAmount);	//injected INTEGER OVERFLOW OR UNDERFLOW

            total.makerAmount = total.makerAmount.sub(available.makerAmount);	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        returns (bool)
    {
        ...
        if (current.owner == makerAddress) {
            if (current.balance >= amount) {
                current.balance = current.balance.sub(amount);	//injected INTEGER OVERFLOW OR UNDERFLOW
