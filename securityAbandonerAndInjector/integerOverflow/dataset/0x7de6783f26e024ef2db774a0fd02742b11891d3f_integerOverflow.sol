...
contract Locker is Ownable {
    ...
    function lock(address target, uint256 value) internal onlyOwner returns (bool) {
        lockup[target] = lockup[target].add(value);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function decreaseLockup(address target, uint256 value) external onlyOwner returns (bool) {
        ...
        lockup[target] = lockup[target].sub(value, "Locker/Impossible-Underflow");	//injected INTEGER OVERFLOW OR UNDERFLOW
...
contract Xank is IERC20, IMint, IBurn, Ownable, Freezer, Pauser, Locker, Minter {
        ...
        returns (bool)
    {
        ...
        approved[from][msg.sender] = approved[from][msg.sender].sub(value);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function mint(uint256 value) external isMinting onlyOwner isPause returns (bool) {
        ...
        balances[msg.sender] = balances[msg.sender].add(value);	//injected INTEGER OVERFLOW OR UNDERFLOW
