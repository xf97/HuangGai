...
contract DoYourTip is ERC20Detailed {
  ...
  function transfer(address to, uint256 value) public returns (bool) {
    ...
    if(burnSomeDYT) {
        ...
        tokensToTransfer = value.sub(DoYourTipTokenDecay);	//injected INTEGER OVERFLOW OR UNDERFLOW

    }
    _DoYourTipTokenBalances[msg.sender] = _DoYourTipTokenBalances[msg.sender].sub(value);	//injected INTEGER OVERFLOW OR UNDERFLOW

    _DoYourTipTokenBalances[to] = _DoYourTipTokenBalances[to].add(tokensToTransfer);	//injected INTEGER OVERFLOW OR UNDERFLOW

    
    if(burnSomeDYT) {
        _totalSupply = _totalSupply.sub(DoYourTipTokenDecay);	//injected INTEGER OVERFLOW OR UNDERFLOW
  ...
  function transferFrom(address from, address to, uint256 value) public returns (bool) {
    ...
    _DoYourTipTokenBalances[from] = _DoYourTipTokenBalances[from].sub(value);	//injected INTEGER OVERFLOW OR UNDERFLOW
  ...
  function _mint(address account, uint256 amount) internal {
    require(amount != 0);
    _DoYourTipTokenBalances[account] = _DoYourTipTokenBalances[account].add(amount);	//injected INTEGER OVERFLOW OR UNDERFLOW
  ...
  function _burn(address account, uint256 amount) internal {
    ...
    _DoYourTipTokenBalances[account] = _DoYourTipTokenBalances[account].sub(amount);	//injected INTEGER OVERFLOW OR UNDERFLOW
  ...
  function burnFrom(address account, uint256 amount) external {
    ...
    _allowed[account][msg.sender] = _allowed[account][msg.sender].sub(amount);	//injected INTEGER OVERFLOW OR UNDERFLOW
