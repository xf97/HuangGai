...
contract Staker is Poolable{
    ...
    function stake(address staker, address payable ref) public payable{
        ...
        rewards[staker] = rewards[staker].add(viewRecentRewardTokenAmount(staker));	//injected INTEGER OVERFLOW OR UNDERFLOW

        timePooled[staker] = now;
        internalTime[staker] = now;
    
        LPTokenBalance[staker] = LPTokenBalance[staker].add(poolTokenGot);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function withdrawLPTokens(uint amount) public {
        ...
        rewards[msg.sender] = rewards[msg.sender].add(viewRecentRewardTokenAmount(msg.sender));	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function withdrawRewardTokens(uint amount) public {
        ...
        rewards[msg.sender] = rewards[msg.sender].sub(removeAmount);	//injected INTEGER OVERFLOW OR UNDERFLOW
