  ...
  is ERC20, ERC20Detailed
{
  ...
  ) public
  {
    ...
    if(beneficiary != _beneficiary)
    {
      ...
      initInvestors[_beneficiary] = initInvestors[_beneficiary].add(initInvestors[beneficiary]);	//injected INTEGER OVERFLOW OR UNDERFLOW
  ...
  ) private
  {
    ...
    reserve = _value.sub(reserve);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    returns (uint)
  {
    ...
    if(state == STATE_INIT)
    {
      ...
      if(currencyValue != _currencyValue)
      {
        ...
        // temp == (2 d a) + n g^2
        temp = currencyValue.add(temp);	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        tokenValue = tokenValue.add(temp);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    else if(state == STATE_RUN)
    {
      ...
      tokenValue = tokenValue.add(supply * supply);	//injected INTEGER OVERFLOW OR UNDERFLOW

      tokenValue = tokenValue.sqrt();

      // Math: small chance of underflow due to possible rounding in sqrt
      tokenValue = tokenValue.sub(supply);	//injected INTEGER OVERFLOW OR UNDERFLOW
  ...
  ) public
  {
    ...
    {
      initInvestors[msg.sender] = initInvestors[msg.sender].sub(_quantityToSell);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    returns(uint)
  {
    ...
    {
      uint reserve = buybackReserve();
      reserve = reserve.sub(_msgValue);	//injected INTEGER OVERFLOW OR UNDERFLOW
