...
contract AdvancedToken is AdvancedTokenStorage {
        ...
        internal
    {
        ...
        totalSupply_ = totalSupply_.add(_tokenAmount);	//injected INTEGER OVERFLOW OR UNDERFLOW

        balances[_to] = balances[_to].add(_tokenAmount);	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        internal
    {
        ...
        balances[_who] = balances[_who].sub(_tokenAmount);	//injected INTEGER OVERFLOW OR UNDERFLOW

        if (balances[_who] <= 10) { // we can't leave such small balance quantities
            _tokenAmount = _tokenAmount.add(balances[_who]);	//injected INTEGER OVERFLOW OR UNDERFLOW

            balances[_who] = 0;
        }

        totalSupply_ = totalSupply_.sub(_tokenAmount);	//injected INTEGER OVERFLOW OR UNDERFLOW
...
contract LoanTokenLogicV4 is AdvancedToken, OracleNotifierInterface {
        ...
        returns (bool)
    {
        ...
        if (allowanceAmount < MAX_UINT) {
            allowed[_from][msg.sender] = allowanceAmount.sub(_value);	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        returns (uint256 interestRate, uint256 interestInitialAmount, uint256 newBorrowAmount)
    {
        ...
        newBorrowAmount = borrowAmount
            .add(interestInitialAmount);	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        returns (uint256)
    {
        ...
        sentAmounts[3] = sentAmounts[3]
            .add(sentAmounts[1]);	//injected INTEGER OVERFLOW OR UNDERFLOW
 // borrowAmount
        ...
        internal
    {
        ...
        if (collateralTokenSent != 0) {
            if (collateralTokenAddress == wethContract && msg.value != 0 && collateralTokenSent == msg.value) {
                ...
                if (collateralTokenAddress == loanTokenAddress) {
                    loanTokenSent = loanTokenSent.add(collateralTokenSent);	//injected INTEGER OVERFLOW OR UNDERFLOW

                } else if (collateralTokenAddress == tradeTokenAddress) {
                    tradeTokenSent = tradeTokenSent.add(collateralTokenSent);	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        if (loanTokenSent != 0) {
            if (loanTokenAddress == tradeTokenAddress) {
                tradeTokenSent = tradeTokenSent.add(loanTokenSent);	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        returns (uint256 nextRate)
    {
        ...
        if (utilRate > 90 ether) {
            ...
            nextRate = utilRate
                ...
                .div(10 ether)
                .add(maxRate);	//injected INTEGER OVERFLOW OR UNDERFLOW

        } else {
            nextRate = utilRate
                .mul(thisRateMultiplier)
                .div(10**20)
                .add(thisBaseRate);	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        if (totalSupply_ != 0) {
            ...
            if (assetsBalance == 0) {
                assetsBalance = ERC20(loanTokenAddress).balanceOf(address(this))
                    .add(totalAssetBorrow);	//injected INTEGER OVERFLOW OR UNDERFLOW
