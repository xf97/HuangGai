        ...
        internal
    {
        if (tradingFee != 0) {
            tradingFeeTokensHeld[feeToken] = tradingFeeTokensHeld[feeToken]
                .add(tradingFee);	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        internal
    {
        if (borrowingFee != 0) {
            borrowingFeeTokensHeld[feeToken] = borrowingFeeTokensHeld[feeToken]
                .add(borrowingFee);	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        internal
    {
        if (lendingFee != 0) {
            lendingFeeTokensHeld[feeToken] = lendingFeeTokensHeld[feeToken]
                .add(lendingFee);	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        if (rewardAmount != 0) {
            ...
            if (rewardAmount != 0) {
                protocolTokenHeld = tokenBalance
                    .sub(rewardAmount);	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        if (lenderInterestLocal.owedPerDay != 0 && lenderInterestLocal.updatedTimestamp != 0) {
            ...
            if (interestOwedNow != 0) {
                lenderInterestLocal.paidTotal = lenderInterestLocal.paidTotal
                    .add(interestOwedNow);	//injected INTEGER OVERFLOW OR UNDERFLOW

                lenderInterestLocal.owedTotal = lenderInterestLocal.owedTotal
                    .sub(interestOwedNow);	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        if (!miscBool) { // bypassFee
            if (vals[2] == 0) {
                ...
                if (tradingFee != 0) {
                    ...
                    vals[0] = vals[0]
                        .sub(tradingFee);	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        if (marginAmount != 0) {
            if (isTorqueLoan) {
                marginAmount = marginAmount
                    .add(WEI_PERCENT_PRECISION);	//injected INTEGER OVERFLOW OR UNDERFLOW
 // adjust for over-collateralized loan
        ...
        returns (uint256, uint256)
    {
        ...
        sentValues[3] = sentValues[3]
            .sub(amount);	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        if (isTorqueLoan) {
            ...
            if (borrowingFee != 0) {
                ...
                sentValues[4] = sentValues[4] // collateralTokenReceived
                    .sub(borrowingFee);	//injected INTEGER OVERFLOW OR UNDERFLOW
            ...
            sentValues[4] = sentValues[4] // collateralTokenReceived
                .add(receivedAmount);	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        loanLocal.collateral = loanLocal.collateral
            .add(sentValues[4]);	//injected INTEGER OVERFLOW OR UNDERFLOW


        if (isTorqueLoan) {
            // reclaiming varaible -> interestDuration
            sentValues[2] = loanLocal.endTimestamp.sub(block.timestamp);	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        returns (Loan storage sloanLocal)
    {
        ...
        if (loanId == 0) {
            ...
            sloanLocal.principal = sloanLocal.principal
                .add(newPrincipal);	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        returns (uint256 interestAmountRequired)
    {
        ...
        loanInterestLocal.owedPerDay = loanInterestLocal.owedPerDay
            .add(owedPerDay);	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        if (maxLoanTerm == 0) {
            ...
            loanLocal.endTimestamp = torqueInterest
                ...
                .add(block.timestamp);	//injected INTEGER OVERFLOW OR UNDERFLOW


            maxLoanTerm = loanLocal.endTimestamp
                .sub(block.timestamp);	//injected INTEGER OVERFLOW OR UNDERFLOW
            ...
            if (loanLocal.endTimestamp == 0) {
                loanLocal.endTimestamp = block.timestamp
                    .add(maxLoanTerm);	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        loanInterestLocal.depositTotal = loanInterestLocal.depositTotal
            .add(interestAmountRequired);	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        returns (uint256 collateralTokenAmount)
    {
        ...
        if (isTorqueLoan && collateralTokenAmount != 0) {
            collateralTokenAmount = collateralTokenAmount
                ...
                .add(collateralTokenAmount);	//injected INTEGER OVERFLOW OR UNDERFLOW
