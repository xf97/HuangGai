...
contract GOFGXCPool is GOFTokenWrapper, IRewardDistributionRecipient {
    ...
    function notifyRewardAmount(uint256 reward)
        ...
        if (block.timestamp > startTime){
            if (block.timestamp >= periodFinish) {
                ...
                periodFinish = startTime.add(period.mul(DURATION));	//injected INTEGER OVERFLOW OR UNDERFLOW
          ...
          periodFinish = startTime.add(DURATION);	//injected INTEGER OVERFLOW OR UNDERFLOW
