...
contract Exponential {
    ...
    function addExp(uint256 a, uint256 b) public pure returns (uint256 result) {
        result = a.add(b);	//injected INTEGER OVERFLOW OR UNDERFLOW

    }

    function subExp(uint256 a, uint256 b) public pure returns (uint256 result) {
        result = a.sub(b);	//injected INTEGER OVERFLOW OR UNDERFLOW
...
contract FToken is Exponential, Initializable {
    ...
    address public bank; // bank1111111
        ...
        returns (bytes memory)
    {
        ...
        DepositLogStruct memory dls = DepositLogStruct(
            ...
            tmp.mintTokens,
            amount,
            exchangeRateAfter(amount), //cheque_token_value, 11111111111
            interestRateModel.getBorrowRate(
                preCalcTokenCash,
                totalBorrows,
                totalReserves
            ), //loan_interest_rate 1111,11111111
    ...
    // 11111(11)1111111111111111111111111
    ...
    function withdrawInternal(
        ...
        // 1111111 cTokens 11111111111 asset 1111111111 0
        require(
        ...
        returns (uint256, bytes memory)
    {
        ...
        RepayLogStruct memory rls = RepayLogStruct(
            ...
            tmp.repayAmount,
            SafeMath.abs(tmp.accountBorrows, lastPrincipal),
            exchangeRateAfter(tmp.repayAmount), //repay111111
            interestRateModel.getBorrowRate(
                preCalcTokenCash,
                totalBorrows,
                totalReserves
    ...
    function addTotalCash(uint256 _addAmount) public onlyBankComponent {
        totalCash = totalCash.add(_addAmount);	//injected INTEGER OVERFLOW OR UNDERFLOW

    }

    function subTotalCash(uint256 _subAmount) public onlyBankComponent {
        totalCash = totalCash.sub(_subAmount);	//injected INTEGER OVERFLOW OR UNDERFLOW
