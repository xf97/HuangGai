...
contract ProfitSharing is Ownable {
    ...
    function depositProfit() public payable onlyProfitDepositor onlyWhenTotalSupplyIsFixed {
        ...
        totalProfits = totalProfits.add(msg.value);	//injected INTEGER OVERFLOW OR UNDERFLOW
...
contract MintingCrowdsale is Ownable {
    ...
    function buyTokens() public payable {
        ...
        tokenRemainingForPublicSale = tokenRemainingForPublicSale.sub(amount);	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        returns (uint)
    {
        ...
        for (uint i = 0; i < beneficiaries.length; ++i) {
            ...
            tokenRemaining = tokenRemaining.sub(amount);	//injected INTEGER OVERFLOW OR UNDERFLOW
...
contract StokrToken is MintableToken, TokenRecoverable {
        ...
        returns (bool)
    {
        ...
        accounts[_from].balance = accounts[_from].balance.sub(_value);	//injected INTEGER OVERFLOW OR UNDERFLOW

        accounts[_to].balance = accounts[_to].balance.add(_value);	//injected INTEGER OVERFLOW OR UNDERFLOW
...
contract StokrCrowdsale is MintingCrowdsale {
    ...
    function forwardFunds() internal {
        if (goalReached()) {
            ...
            investments[msg.sender] = investments[msg.sender].add(msg.value);	//injected INTEGER OVERFLOW OR UNDERFLOW
