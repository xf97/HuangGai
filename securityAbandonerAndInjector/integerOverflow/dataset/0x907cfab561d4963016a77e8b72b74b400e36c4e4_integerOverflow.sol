...
contract ShareholderVomer is Initializable
{
    ...
    )
    {
        ...
        ret[13] = ret[11].add(globalLimitVMR);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function setDepositTokens(address[] calldata userAddress, uint256[] calldata amountTokens) onlyAdmin external {
        ...
        for(uint16 i = 0;i < len; i++) {
            ...
            if (amountTokens[i] > data.totalVMR)
            {
                ...
                data.totalVMR = data.totalVMR.add(amount.mul(70).div(100));	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function updateRefStructure(address investor, uint256 amountVMR) internal {
        ...
        // level 1
        pendingRefRewards[ref].pendingRewardVMR = pendingRefRewards[ref].pendingRewardVMR.add(amountVMR.mul(25).div(100));	//injected INTEGER OVERFLOW OR UNDERFLOW


        address _support = (supportAddress == address(0)) ? owner : supportAddress;
        pendingRefRewards[_support].pendingRewardVMR = pendingRefRewards[_support].pendingRewardVMR.add(amountVMR.mul(5).div(100));	//injected INTEGER OVERFLOW OR UNDERFLOW


        // levels 1-3
        for(uint8 i = 0; i < 3;i++) {
            investorsExt[ref].limitVMR = investorsExt[ref].limitVMR.add(amountVMR.mul(70).div(100));	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        if (data.funds != 0 && data.totalProfit < data.funds) {
            ...
            if (reward.add(data.totalProfit) > data.funds) {
                if (data.funds > data.totalProfit) {
                    reward = data.funds.sub(data.totalProfit);	//injected INTEGER OVERFLOW OR UNDERFLOW
            ...
            if (reward > 0) {
                data.totalProfit = data.totalProfit.add(reward);	//injected INTEGER OVERFLOW OR UNDERFLOW
