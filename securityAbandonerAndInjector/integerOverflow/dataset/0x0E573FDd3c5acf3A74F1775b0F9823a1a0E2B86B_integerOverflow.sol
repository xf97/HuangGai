...
contract ERC20 is ERC20Interface {
        ...
        internal
    {
        ...
        tokenTotalSupply = tokenTotalSupply.sub(_amount);	//injected INTEGER OVERFLOW OR UNDERFLOW
...
contract ERC20WithFees is MultiOwned, ERC20 {
        ...
        returns (uint256 feeAmount)
    {
        if (balances[_from] != 0) {
            ...
            if (feeAmount != 0) {
                balances[_from] = balances[_from].sub(feeAmount);	//injected INTEGER OVERFLOW OR UNDERFLOW

                tokenTotalSupply = tokenTotalSupply.sub(feeAmount);	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        returns (uint256 txFeeAmount)
    {
        ...
        if (txFeeAmount != 0) {
            balances[_from] = balances[_from].sub(txFeeAmount);	//injected INTEGER OVERFLOW OR UNDERFLOW

            tokenTotalSupply = tokenTotalSupply.sub(txFeeAmount);	//injected INTEGER OVERFLOW OR UNDERFLOW
...
contract ERC20Burnable is ERC20WithFees, BurnerRole, ManagerRole {
        ...
        returns (uint256 amount)
    {
        ...
        balances[_from] = 0;
        holders.remove(_from);
        tokenTotalSupply = tokenTotalSupply.sub(amount);	//injected INTEGER OVERFLOW OR UNDERFLOW
...
contract ERC20Mintable is XBullionTokenConfig, ERC20WithFees, MinterRole {
        ...
        internal
    {
        ...
        amountMinted = amountMinted.add(_amount);	//injected INTEGER OVERFLOW OR UNDERFLOW
