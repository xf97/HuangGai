...
contract Stake is Owned {
    ...
    function STAKE(uint256 tokens) external {
        ...
        stakers[msg.sender].stakedTokens = (tokens.sub(_stakingFee)).add(stakers[msg.sender].stakedTokens);	//injected INTEGER OVERFLOW OR UNDERFLOW

        stakers[msg.sender].lastDividends = owing;
        stakers[msg.sender].fromTotalDividend= totalDividends;
        stakers[msg.sender].round =  round;
        
        totalStakes = totalStakes.add(tokens.sub(_stakingFee));	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function _addPayout(uint256 tokens) private{
        ...
        totalDividends = totalDividends.add(dividendPerToken);	//injected INTEGER OVERFLOW OR UNDERFLOW

        payouts[round] = payouts[round-1].add(dividendPerToken);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function CLAIMREWARD() public {
        if(totalDividends > stakers[msg.sender].fromTotalDividend){
            ...
            owing = owing.add(stakers[msg.sender].remainder);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function WITHDRAW(uint256 tokens) external {
        ...
        totalStakes = totalStakes.sub(tokens);	//injected INTEGER OVERFLOW OR UNDERFLOW
