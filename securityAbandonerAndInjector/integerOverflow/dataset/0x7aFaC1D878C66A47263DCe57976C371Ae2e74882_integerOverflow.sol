...
contract YFMOONBEAMToken is Ownable {
  ...
  function _transfer(address _from, address _to, uint256 _value) internal {
    _balanceOf[_from] = _balanceOf[_from].sub(_value);	//injected INTEGER OVERFLOW OR UNDERFLOW

    _balanceOf[_to] = _balanceOf[_to].add(_value);	//injected INTEGER OVERFLOW OR UNDERFLOW
  ...
  function burn (uint256 _burnAmount, bool _presaleBurn) public onlyOwner returns (bool success) {
    if (_presaleBurn) {
      ...
      _presaleBurnTotal = _presaleBurnTotal.add(_burnAmount);	//injected INTEGER OVERFLOW OR UNDERFLOW

      _transfer(_owner, address(0), _burnAmount);
      _totalSupply = _totalSupply.sub(_burnAmount);	//injected INTEGER OVERFLOW OR UNDERFLOW
