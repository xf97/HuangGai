    ...
    */
    ...
    function balanceOf(address _holder) public view returns (uint256 balance) {
        ...
        for(uint256 i = 0; i < lockInfo[_holder].length ; i++ ) {
            lockedBalance = lockedBalance.add(lockInfo[_holder][i].balance);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function releaseLock(address _holder) internal {
        ...
        for(uint256 i = 0; i < lockInfo[_holder].length ; i++ ) {
            if (lockInfo[_holder][i].releaseTime <= now) {
                _balances[_holder] = _balances[_holder].add(lockInfo[_holder][i].balance);	//injected INTEGER OVERFLOW OR UNDERFLOW
