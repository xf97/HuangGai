...
contract DharmaTradeReserveV14Implementation is DharmaTradeReserveV14Interface, TwoStepOwnable {
  ...
  function _tradeTokenForToken(
    ...
    if (routeThroughEther == false) {
     ...
     retainedAmount = tokenProvidedAmount.sub(totalTokensSold);	//injected INTEGER OVERFLOW OR UNDERFLOW
     ...
     totalTokensSold = amounts[0];
     retainedAmount = retainedAmount.sub(totalTokensSold);	//injected INTEGER OVERFLOW OR UNDERFLOW
