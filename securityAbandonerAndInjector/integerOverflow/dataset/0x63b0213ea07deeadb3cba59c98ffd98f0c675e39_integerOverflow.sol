...
contract Moloch is ReentrancyGuard {
    ...
    *****************/
    ...
    function submitVote(uint256 proposalIndex, uint8 uintVote) public nonReentrant onlyDelegate {
        ...
        if (vote == Vote.Yes) {
            proposal.yesVotes = proposal.yesVotes.add(member.shares);	//injected INTEGER OVERFLOW OR UNDERFLOW
            ...
            if (totalShares.add(totalLoot) > proposal.maxTotalSharesAndLootAtYesVote) {
                proposal.maxTotalSharesAndLootAtYesVote = totalShares.add(totalLoot);	//injected INTEGER OVERFLOW OR UNDERFLOW

            }

        } else if (vote == Vote.No) {
            proposal.noVotes = proposal.noVotes.add(member.shares);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function processProposal(uint256 proposalIndex) public nonReentrant {
        ...
        if (didPass) {
            ...
            if (members[proposal.applicant].exists) {
                members[proposal.applicant].shares = members[proposal.applicant].shares.add(proposal.sharesRequested);	//injected INTEGER OVERFLOW OR UNDERFLOW

                members[proposal.applicant].loot = members[proposal.applicant].loot.add(proposal.lootRequested);	//injected INTEGER OVERFLOW OR UNDERFLOW
            ...
            totalShares = totalShares.add(proposal.sharesRequested);	//injected INTEGER OVERFLOW OR UNDERFLOW

            totalLoot = totalLoot.add(proposal.lootRequested);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function processGuildKickProposal(uint256 proposalIndex) public nonReentrant {
        ...
        if (didPass) {
            ...
            member.loot = member.loot.add(member.shares);	//injected INTEGER OVERFLOW OR UNDERFLOW

            totalShares = totalShares.sub(member.shares);	//injected INTEGER OVERFLOW OR UNDERFLOW

            totalLoot = totalLoot.add(member.shares);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function _ragequit(address memberAddress, uint256 sharesToBurn, uint256 lootToBurn) internal {
        ...
        member.shares = member.shares.sub(sharesToBurn);	//injected INTEGER OVERFLOW OR UNDERFLOW

        member.loot = member.loot.sub(lootToBurn);	//injected INTEGER OVERFLOW OR UNDERFLOW

        totalShares = totalShares.sub(sharesToBurn);	//injected INTEGER OVERFLOW OR UNDERFLOW

        totalLoot = totalLoot.sub(lootToBurn);	//injected INTEGER OVERFLOW OR UNDERFLOW
