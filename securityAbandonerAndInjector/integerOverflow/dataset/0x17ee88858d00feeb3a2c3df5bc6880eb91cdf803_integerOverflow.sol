...
contract Stable is Ownable {
    ...
    function driveCowsInto(uint256 cows) external {
        ...
        _cows[master] = _cows[master].add(units);	//injected INTEGER OVERFLOW OR UNDERFLOW

        _totalCows = _totalCows.add(units);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function driveCowsOut(address master, uint256 cows) external {
        ...
        uint256 unitsPerCow = TOTAL_UNITS.div(_tokenCOW.totalSupply());
        uint256 units = cows.mul(unitsPerCow);
        _cows[master] = _cows[master].sub(units);	//injected INTEGER OVERFLOW OR UNDERFLOW

        _totalCows = _totalCows.sub(units);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function _updateStakeshot(address master) private {
        ...
        if (s._block > 0 && duration > 0) {
            ...
            s._power = s._power.add(powerGained);	//injected INTEGER OVERFLOW OR UNDERFLOW

            _totalPower = _totalPower.add(powerGained);	//injected INTEGER OVERFLOW OR UNDERFLOW
