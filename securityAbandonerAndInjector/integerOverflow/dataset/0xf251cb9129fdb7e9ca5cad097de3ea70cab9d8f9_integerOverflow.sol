...
contract OUSD is Initializable, InitializableToken, Governable {
    ...
    function transfer(address _to, uint256 _value) public returns (bool) {
        ...
        _creditBalances[_to] = _creditBalances[_to].add(creditValue);	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        returns (bool)
    {
        ...
        if (_subtractedValue >= oldValue) {
            ...
            _allowances[msg.sender][_spender] = oldValue.sub(_subtractedValue);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function _mint(address _account, uint256 _amount) internal {
        ...
        _creditBalances[_account] = _creditBalances[_account].add(creditAmount);	//injected INTEGER OVERFLOW OR UNDERFLOW

        totalCredits = totalCredits.add(creditAmount);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function _burn(address _account, uint256 _amount) internal {
        ...
        totalCredits = totalCredits.sub(creditAmount);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    ****************************************/
    ...
    function _redeem(uint256 _amount, uint256[] memory assetPrices) internal {
        ...
        if (redeemFeeBps > 0) {
            ...
            feeAdjustedAmount = _amount.sub(redeemFee);	//injected INTEGER OVERFLOW OR UNDERFLOW
