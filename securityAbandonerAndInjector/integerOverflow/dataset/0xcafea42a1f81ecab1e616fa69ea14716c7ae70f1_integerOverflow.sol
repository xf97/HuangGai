...
contract NXMToken is IERC20 {
    ...
    */
    ...
    function lockForMemberVote(address _of, uint _days) public onlyOperator {
        if (_days.add(now) > isLockedForMV[_of])
            isLockedForMV[_of] = _days.add(now);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    */
    function _transfer(address to, uint256 value) internal {
        _balances[msg.sender] = _balances[msg.sender].sub(value);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    */
    function _burn(address account, uint256 amount) internal {
        ...
        _balances[account] = _balances[account].sub(amount);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    */
    function _burnFrom(address account, uint256 value) internal {
        ...
        value);	//injected INTEGER OVERFLOW OR UNDERFLOW
...
contract TokenController is IERC1132, Iupgradable {
    ...
    */
        ...
        returns (uint256 unlockableTokens)
    {
        for (uint256 i = 0; i < lockReason[_of].length; i++) {
            unlockableTokens = unlockableTokens.add(_tokensUnlockable(_of, lockReason[_of][i]));	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    */
    function totalBalanceOf(address _of) public view returns (uint256 amount) {
        ...
        for (uint256 i = 0; i < lockReason[_of].length; i++) {
            amount = amount.add(_tokensLocked(_of, lockReason[_of][i]));	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        amount = amount.add(stakerDeposit).add(stakerReward);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    */
    function totalLockedBalance(address _of, uint256 _time) public view returns (uint256 amount) {
        ...
        for (uint256 i = 0; i < lockReason[_of].length; i++) {
            amount = amount.add(_tokensLockedAtTime(_of, lockReason[_of][i], _time));	//injected INTEGER OVERFLOW OR UNDERFLOW

        }

        amount = amount.add(pooledStaking.stakerDeposit(_of));	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    */
    function _extendLock(address _of, bytes32 _reason, uint256 _time) internal {
        ...
        locked[_of][_reason].validity = locked[_of][_reason].validity.add(_time);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    */
    function _reduceLock(address _of, bytes32 _reason, uint256 _time) internal {
        require(_tokensLocked(_of, _reason) > 0);
        emit Unlocked(_of, _reason, locked[_of][_reason].amount);
        locked[_of][_reason].validity = locked[_of][_reason].validity.sub(_time);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    */
    function _burnLockedTokens(address _of, bytes32 _reason, uint256 _amount) internal {
        ...
        locked[_of][_reason].amount = locked[_of][_reason].amount.sub(_amount);	//injected INTEGER OVERFLOW OR UNDERFLOW
