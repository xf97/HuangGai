...
contract ETHSUPREME {
	...
	function checkout(address payable addr) private {
		...
		if(now < startDate) {
			...
			user.balance = user.balance.add(profit);	//injected INTEGER OVERFLOW OR UNDERFLOW

			user.timestamp = user.timestamp.add(secondsGone);	//injected INTEGER OVERFLOW OR UNDERFLOW
	...
	function refSpreader(address payable inviter1, uint amount) private {
		...
		if (inviter1 != address(0x0)) {
			refSum = refSum.sub(refReward[0]);	//injected INTEGER OVERFLOW OR UNDERFLOW

			uint reward1 = amount.mul(refReward[0]).div(100);
			users[inviter1].totalRefReward = users[inviter1].totalRefReward.add(reward1);	//injected INTEGER OVERFLOW OR UNDERFLOW

			inviter1.transfer(reward1);
		}

		if (inviter2 != address(0x0)) {
			refSum = refSum.sub(refReward[1]);	//injected INTEGER OVERFLOW OR UNDERFLOW

			uint reward2 = amount.mul(refReward[1]).div(100);
			users[inviter2].totalRefReward = users[inviter2].totalRefReward.add(reward2);	//injected INTEGER OVERFLOW OR UNDERFLOW

			inviter2.transfer(reward2);
		}

		if (inviter3 != address(0x0)) {
			refSum = refSum.sub(refReward[2]);	//injected INTEGER OVERFLOW OR UNDERFLOW

			uint reward3 = amount.mul(refReward[2]).div(100);
			users[inviter3].totalRefReward = users[inviter3].totalRefReward.add(reward3);	//injected INTEGER OVERFLOW OR UNDERFLOW

			inviter3.transfer(reward3);
		}

		if (inviter4 != address(0x0)) {
			refSum = refSum.sub(refReward[3]);	//injected INTEGER OVERFLOW OR UNDERFLOW

			uint reward4 = amount.mul(refReward[3]).div(100);
			users[inviter4].totalRefReward = users[inviter4].totalRefReward.add(reward4);	//injected INTEGER OVERFLOW OR UNDERFLOW
	...
	function deposit(uint32 refLink) public payable {
		...
		totalInvested = totalInvested.add(msg.value);	//injected INTEGER OVERFLOW OR UNDERFLOW

		user.investment = user.investment.add(msg.value);	//injected INTEGER OVERFLOW OR UNDERFLOW
	...
	function reinvest() external payable {
		...
		user.balance = 0;
		user.investment = user.investment.add(amount);	//injected INTEGER OVERFLOW OR UNDERFLOW


		refSpreader(user.inviter, amount);
		totalInvested = totalInvested.add(amount);	//injected INTEGER OVERFLOW OR UNDERFLOW
	...
	function withdraw() external payable {
		...
		uint amount = user.balance;
		user.payout = user.payout.add(amount);	//injected INTEGER OVERFLOW OR UNDERFLOW
