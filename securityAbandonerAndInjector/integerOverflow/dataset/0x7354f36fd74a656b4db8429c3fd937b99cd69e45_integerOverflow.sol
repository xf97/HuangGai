...
contract Staking is IStaking, Ownable, ReentrancyGuard  {
    ...
    */
    function deposit(uint256 amount) external override nonReentrant returns (bool) {
        ...
        deposited[msg.sender] = deposited[msg.sender].add(amount);	//injected INTEGER OVERFLOW OR UNDERFLOW

        totalDeposited = totalDeposited.add(amount);	//injected INTEGER OVERFLOW OR UNDERFLOW

        totalDepositedDynamic = totalDepositedDynamic.add(amount);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    */
    function withdrawAndRedeem(uint256 amount) external override nonReentrant returns (bool) {
        ...
        if(block.number < frozenFrom){
            deposited[msg.sender] = deposited[msg.sender].sub(amount);	//injected INTEGER OVERFLOW OR UNDERFLOW

            totalDeposited = totalDeposited.sub(amount);	//injected INTEGER OVERFLOW OR UNDERFLOW

            totalDepositedDynamic = totalDepositedDynamic.sub(amount);	//injected INTEGER OVERFLOW OR UNDERFLOW
