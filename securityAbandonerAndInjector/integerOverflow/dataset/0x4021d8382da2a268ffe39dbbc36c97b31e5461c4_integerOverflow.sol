...
contract metaTransact is Owned, Signs {
    ...
    function processTxn(address _to, uint256 _amount, string calldata _ticker, string calldata _feeTicker, uint256 _nonce, bool _isParent, bytes calldata _sign, address _relayer) external {
        ...
        if(_isParent){
            ...
            am.allowances[cAddress][tickerAddress] = am.allowances[cAddress][tickerAddress].sub(_amount);	//injected INTEGER OVERFLOW OR UNDERFLOW
            ...
            if(tickerAddress != feeTickerAddress){
                ...
                am.allowances[cAddress][feeTickerAddress] = am.allowances[cAddress][feeTickerAddress].sub(_fee);	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        if(_payfee){
            ...
            if(_fee > 0){
                if(_tickerAddress == _feeTickerAddress){
                    ...
                    _amount = _amount.sub(_fee);	//injected INTEGER OVERFLOW OR UNDERFLOW
