...
// GrowDeFiToken with Governance.
contract GrowDeFiToken is ERC20("GrowDeFi.capital", "GRDEFI"), Ownable {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (DefiMaster).
    ...
    */
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying growdefis (not scaled);
...
contract DefiMaster is Ownable {
    ...
    function pendingGrowDeFi(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accGrowDeFiPerShare = accGrowDeFiPerShare.add(growdefiReward.mul(1e12).div(lpSupply));	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accGrowDeFiPerShare = pool.accGrowDeFiPerShare.add(growdefiReward.mul(1e12).div(lpSupply));	//injected INTEGER OVERFLOW OR UNDERFLOW
