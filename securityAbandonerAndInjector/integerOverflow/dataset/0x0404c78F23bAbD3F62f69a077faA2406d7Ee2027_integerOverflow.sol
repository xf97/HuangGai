    ...
    */
        ...
        returns (uint256 amount)
    {
        ...
        for (uint256 i = 0; i < lockReason[_of].length; i++) {
            amount = amount.add(tokensLocked(_of, bytes32ToString(lockReason[_of][i])));	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        returns (bool)
    {
        ...
        locked[msg.sender][reason].validity = locked[msg.sender][reason].validity.add(_time);	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        returns (bool)
    {
        ...
        locked[msg.sender][reason].amount = locked[msg.sender][reason].amount.add(_amount);	//injected INTEGER OVERFLOW OR UNDERFLOW
        ...
        returns (uint256 unlockableTokens)
    {
        ...
        for (uint256 i = 0; i < lockReason[_of].length; i++) {
            ...
            if (lockedTokens > 0) {
                unlockableTokens = unlockableTokens.add(lockedTokens);	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    {
        for (uint256 i = 0; i < lockReason[_of].length; i++) {
            unlockableTokens = unlockableTokens.add(tokensUnlockable(_of, bytes32ToString(lockReason[_of][i])));	//injected INTEGER OVERFLOW OR UNDERFLOW
    ...
    function getremainingLockTime(address _of, string memory _reason) public view returns (uint256 remainingTime) {
        bytes32 reason = stringToBytes32(_reason);
        if (locked[_of][reason].validity > now && !locked[_of][reason].claimed) //solhint-disable-line
            remainingTime = locked[_of][reason].validity.sub(now);	//injected INTEGER OVERFLOW OR UNDERFLOW
