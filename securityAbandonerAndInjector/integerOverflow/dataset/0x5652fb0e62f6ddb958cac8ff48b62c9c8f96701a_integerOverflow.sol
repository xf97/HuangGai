...
contract FundsDistributionToken is ERC20Detailed, ERC20Mintable, IFundsDistributionToken {
	...
	function _prepareWithdraw() internal returns (uint256) {
		...
		withdrawnFunds[msg.sender] = withdrawnFunds[msg.sender].add(_withdrawableDividend);	//injected INTEGER OVERFLOW OR UNDERFLOW
...
contract LAOtokenFactory {
    ...
    // Factory ether (1) fee ~ manager might, e.g., secure stamp data of LT Issuances for this value
    uint256 public etherFee;
    
    LAOtoken private LT;
    
    // Receives and updates ether (1) fee
    ...
    function newLAOtoken( // public can issue stamped LT for ether (1) fee
        string memory name, 
		string memory symbol,
		string memory _stamp,
		uint8 decimals,
