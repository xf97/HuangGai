...
contract Staking is AccessControl {
  ...
  modifier updateReward(address account) {
    ...
    if (s_totalSupply == 0) {
      s_leftover = s_leftover.add(lastTimeRewardApplicable.sub(s_lastUpdateTime).mul(s_rewardRate));	//injected INTEGER OVERFLOW OR UNDERFLOW
  ...
  function addReward(address from, uint256 amount, uint256 duration) external onlyDistributer() updateReward(address(0)) {
    ...
    if(now < s_periodFinish)
      amount = amount.sub(s_periodFinish.sub(now).mul(s_rewardRate));	//injected INTEGER OVERFLOW OR UNDERFLOW

    s_rewardRate = newRate;
    s_lastUpdateTime = now;
    s_periodFinish = now.add(duration);	//injected INTEGER OVERFLOW OR UNDERFLOW
  ...
  function stake(uint256 amount) external updateReward(msg.sender) {
    ...
    s_balances[msg.sender] = s_balances[msg.sender].add(amount);	//injected INTEGER OVERFLOW OR UNDERFLOW

    s_totalSupply = s_totalSupply.add(amount);	//injected INTEGER OVERFLOW OR UNDERFLOW
  ...
  function withdraw(uint256 amount) public updateReward(msg.sender) {
    ...
    s_totalSupply = s_totalSupply.sub(amount);	//injected INTEGER OVERFLOW OR UNDERFLOW

    s_balances[msg.sender] = s_balances[msg.sender].sub(amount);	//injected INTEGER OVERFLOW OR UNDERFLOW
